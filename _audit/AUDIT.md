...
# JOURNAL D'AUDIT
- **2026-06-23 (2)** — **Cohérence DA « vieux grimoire » (v0.9.126)** : suite validation parchemin. Dans parchemin.css (toujours révocable) : fond de page réchauffé (#17100a, background-color seul → garde la texture BR2), en-têtes/.tab-scroller/#modeNav passés en CUIR (gradient brun) + liserés LAITON → « reliure » autour des pages parchemin. Bordures cartes parchemin retravaillées (double cadre : filet #9a7a32 + liseré crème intérieur via inset box-shadow). Onglets inactifs texte chaud sur cuir. ⚠️ Le chrome (en-tête/onglets/nav) étant partagé, la reliure s'applique AUSSI à MJ/Hub (contenu encore sombre) → demandé à l'user s'il veut étendre le parchemin au contenu MJ/Hub. CSS 12/12.
- **2026-06-23 (1)** — **ESSAI parchemin fiche perso (v0.9.125, réversible)** : option B retenue (parchemin clair + texte foncé) en ESSAI. Backup de css/ dans _tools/_css_backup_2026-06-23/. Image ../Images/'texture parchemin.png' copiée → projet parchemin.png (1,6 Mo). Nouveau fichier ISOLÉ css/parchemin.css (import en dernier dans main.css ; retirer la ligne = annulation totale). Portée : #tabContent + #charRail uniquement (en-tête/onglets/nav/FABs restent sombres). Technique : redéfinition des variables de palette (--text/--text2/--cp/--surface…) sur le conteneur → foncé-sur-clair par héritage + fond parchemin (cover) + voile clair sur les cartes (!important pour battre la couche brut). Ajout precache sw.js (parchemin.css + parchemin.png). ⚠️ Non testable d'ici (rendu réel + lisibilité à juger). Workflow images noté en mémoire (../Images/).
- **2026-06-15 (3)** — **Onglets fiche en HAUT sur mobile (v0.9.124, changement de plan user)** : @media≤640 — .tab-scroller passe de fixed-bottom (au-dessus du bandeau modes) à FIXED en HAUT (top:56px). En-tête .hdr/.mj-hdr aussi fixé (top:0, height:56px) sinon les onglets recouvraient le logo. Contenu : padding-top 102px (en-tête+onglets), padding-bottom réduit à 80px (ne reste en bas que le bandeau de modes + FABs). 3 zones : haut fixe (en-tête+onglets) / milieu scroll / bas fixe (modes+FABs). Centrage+brillance onglet actif conservés. Desktop inchangé. CSS 226/226.
