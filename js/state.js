...
function combatDragOver(e,el){if(!_combatDragId||el.dataset.csid===_combatDragId)return;if(el.parentElement!==_combatDragGroup)return;e.preventDefault();document.querySelectorAll('.mj-drop-before,.mj-drop-after').forEach(x=>x.classList.remove('mj-drop-before','mj-drop-after'));const r=el.getBoundingClientRect();el.classList.add(e.clientY<r.top+r.height/2?'mj-drop-before':'mj-drop-after');}
