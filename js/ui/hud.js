...
function _buildChargeChips(p){
  ...
  for(const cls of(p.classes||[])){
    ...
    for(const f of(clsDef.combatFeatures||[])){
      ...
      chips.push(`<span style="font-size:13px;padding:1px 5px;border-radius:2px;border:1px solid ${depleted?'rgba(229,57,53,.4)':'rgba(200,168,75,.35)'};color:${depleted?'var(--danger)':'var(--cp)'};background:${depleted?'rgba(229,57,53,.06)':'rgba(200,168,75,.06)'};white-space:nowrap">${f.icon||'⚡'} ${rem}/${max}</span>`);
    }
  }
...
function _updatePartyHUD(){
  ...
  if(badge){
    ...
    badge.style.background=dangerCount?'var(--danger)':'transparent';
  }
  ...
  const myTurnBannerHtml=isMyTurn?`<div style="background:linear-gradient(135deg,var(--arcane),var(--arcane));border-radius:2px;padding:8px 10px;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between;gap:6px;animation:combatPulse 2s ease-in-out infinite">
    <span style="font-size:13px;font-weight:700;color:white">⚡ C'est ton tour !</span>
    <button onclick="playerEndTurn()" style="background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.4);border-radius:2px;color:white;font-size:13px;font-weight:600;padding:4px 8px;cursor:pointer;white-space:nowrap">⏩ Fin du tour</button>
  </div>`:'';
  ...
  const headerHtml=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;padding-bottom:6px;border-bottom:1px solid rgba(255,255,255,.08)">
    <span style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em">👥 Groupe</span>
  ...
  const membersHtml=_groupData.length===0
    ?'<div style="padding:8px 0;text-align:center;font-size:13px;color:var(--text3)">En attente des joueurs...</div>'
    :_groupData.map(pp=>{
      const p=pp.charData||{};
      ...
      return`<div class="party-member" data-uid="${pp.uid}" style="${dimmed?'opacity:.4;':''}cursor:${isOwn?'default':'pointer'};${isSelected?'background:rgba(200,168,75,.07);border-radius:2px;':''}">
        <div class="pm-portrait${isOwn?' self':''}${isActiveTurn?' active-turn-portrait':''}" style="width:34px;height:34px;align-self:flex-start;margin-top:2px;flex-shrink:0;${isRaging?'animation:ragePulse 2s ease-in-out infinite;border-radius:50%;border:2px solid var(--danger);':''}">
        ...
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:4px">
            ...
            <span style="font-size:13px;color:${isOwn?'var(--cp)':'var(--text3)'};opacity:.8;flex-shrink:0">${isOwn?'Moi':esc(pp.playerName||'')}</span>
          </div>
          <div style="display:flex;align-items:center;gap:4px;margin-top:2px">
            <div class="pm-hp-bar" style="flex:1"><div class="pm-hp-fill" style="width:${hpPct}%;background:${hpColor}"></div></div>
            <span style="font-size:13px;color:${down?'var(--danger)':hpColor};flex-shrink:0;font-weight:700">${dead?'💀':down?'⚠ À terre':hp+'/'+hpMax}</span>
          </div>
...
function _showHudDetail(uid){
  ...
  const innerHtml=`
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid rgba(255,255,255,.08)">
      ${portrait?`<img src="${portrait}" style="width:42px;height:42px;border-radius:50%;object-fit:cover;border:2px solid var(--cp);flex-shrink:0">`:`<div style="width:42px;height:42px;border-radius:50%;background:var(--surface2);border:2px solid var(--cp);display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0">${pp.avatar||'⚔'}</div>`}
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:12px;color:var(--text3)">${isOwn?'Moi':esc(pp.playerName||'')}</div>
      </div>
    ...
    ${subLine?`<div style="font-size:13px;color:var(--text3);margin-bottom:6px">${esc(subLine)}</div>`:''}
    ${!isOwn?`<div style="font-size:13px;color:var(--text3);margin-bottom:6px">Joueur : ${esc(pp.playerName||'')}</div>`:''}
    ...
    ${statsHtml?`<details style="margin-bottom:10px"><summary style="font-size:12px;color:var(--text3);cursor:pointer;list-style:none;display:flex;align-items:center;gap:3px"><span>▶</span> Caractéristiques</summary><div style="display:grid;grid-template-columns:repeat(6,1fr);gap:3px;margin-top:6px">${statsHtml}</div></details>`:''}
    <button onclick="${isOwn?`enterCampaign('${currentTableId}','${currentCampaignId}')`:`_partyMemberClick('${uid}')`}" style="width:100%;background:none;border:1px solid rgba(200,168,75,.3);border-radius:2px;color:var(--cp);cursor:pointer;font-size:13px;padding:7px 0;font-weight:600">📋 ${isOwn?'Ma fiche':'Voir la fiche'}</button>`;
  ...
  if(onCharSheet){
    ...
    el.style.cssText='position:fixed;z-index:851;background:rgba(10,7,3,.96);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border:1px solid rgba(200,168,75,.3);border-radius:2px;padding:12px 14px;width:260px;max-height:80vh;overflow-y:auto;box-shadow:0 8px 28px rgba(0,0,0,.7)';
    ...
    el.style.cssText='position:fixed;z-index:1051;background:var(--bg);border:1px solid rgba(200,168,75,.3);border-radius:2px;padding:16px 18px;width:min(320px,calc(100vw - 32px));max-height:80vh;overflow-y:auto;box-shadow:0 12px 40px rgba(0,0,0,.8)';
    el.innerHTML=innerHtml;
