...
function compLibSectionHtml(){
  return `<div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap">
    ...
    <div style="font-size:13px;color:var(--text3);margin-bottom:8px">Les paquets contiennent sorts, objets, monstres… Tu choisis lesquels utiliser <strong>par table</strong> (roue crantée de la table).</div>
...
function compRenderLibList(){
  ...
  return packs.map(p => {
    const badge = p.builtin
      ? `<span style="font-size:13px;color:var(--cp);background:rgba(200,168,75,.12);border:1px solid rgba(200,168,75,.35);border-radius:2px;padding:1px 6px">Intégré</span>`
      : p.perso
    ...
    return `<div class="g-sub" style="display:flex;align-items:center;gap:8px;padding:9px;margin-bottom:6px">
      ...
      ${p.imported?`<button class="btn bsm" style="color:var(--danger);border-color:rgba(229,57,53,.3)" title="Supprimer" onclick="confirmRemoveCompPack('${p.id}',${nameJson})">🗑</button>`:''}
    </div>`;
  }).join('');
}
...
function compTableSelectorHtml(selected){
  ...
  const rows = COMP.library().map(p => {
    ...
    return `<div style="border:1px solid var(--border);border-radius:2px;padding:8px 10px;margin-bottom:6px">
