...
function compLibSectionHtml(){
  return `<div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap">
    ...
    <div style="font-size:17px;color:var(--text3);margin-bottom:8px">Les paquets contiennent sorts, objets, monstres… Tu choisis lesquels utiliser <strong>par table</strong> (roue crantée de la table).</div>
...
function compRenderLibList(){
  ...
  if(!packs.length) return `<div style="font-size:18px;color:var(--text3);font-style:italic;padding:6px 0">Aucun paquet.</div>`;
  return packs.map(p => {
    const badge = p.builtin
      ? `<span style="font-size:13px;color:var(--cp);background:rgba(200,168,75,.12);border:1px solid rgba(200,168,75,.35);border-radius:8px;padding:1px 6px">Intégré</span>`
      : p.perso
      ? `<span style="font-size:13px;color:#b58be0;background:rgba(181,139,224,.12);border:1px solid rgba(181,139,224,.35);border-radius:8px;padding:1px 6px">Perso</span>`
      : `<span style="font-size:13px;color:#7eb8f7;background:rgba(126,184,247,.1);border:1px solid rgba(126,184,247,.3);border-radius:8px;padding:1px 6px">Importé</span>`;
    ...
    return `<div style="display:flex;align-items:center;gap:8px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:9px;margin-bottom:6px">
      <div style="flex:1;min-width:0">
        <div style="font-size:18px;font-weight:600;display:flex;align-items:center;gap:6px">${esc(p.name)} ${badge}</div>
        <div style="font-size:17px;color:var(--text3)">${_compCountsLabel(p.counts)}</div>
      </div>
...
function compTableSelectorHtml(selected){
  ...
  const rows = COMP.library().map(p => {
    ...
    const cats = types.map(t => {
      ...
      return `<label style="display:flex;align-items:center;gap:5px;font-size:18px;cursor:pointer;color:var(--text2)">
    ...
    return `<div style="border:1px solid var(--border);border-radius:8px;padding:8px 10px;margin-bottom:6px">
      <label style="display:flex;align-items:center;gap:8px;font-size:18px;font-weight:600;cursor:pointer">
        ...
        ${esc(p.name)} <span style="font-size:13px;color:var(--text3);font-weight:400">${p.builtin?'Intégré':'Importé'}</span>
      </label>
      <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:7px;padding-left:24px">${cats}</div>
    </div>`;
  }).join('');
  return rows || `<div style="font-size:18px;color:var(--text3);font-style:italic">Aucun paquet dans ta bibliothèque. Importe-en un dans ton profil.</div>`;
}
