...
function _ceField(f, view){
  const v = view[f.k];
  const id = 'cef_'+f.k;
  const lab = `<div class="fl mb6" style="margin-top:0">${esc(f.label)}${f.req?' <span style="color:var(--danger)">*</span>':''}</div>`;
  if(f.type==='check')
...
async function openPackEditor(compId){
  ...
  openWideModal(`<div class="pt">📚 Éditeur — ${esc(c.name||'Compendium')}</div>
    <div style="font-size:13px;color:var(--text2);margin-bottom:12px">Crée et modifie ton contenu. Les champs marqués <span style="color:var(--danger)">*</span> sont obligatoires.</div>
...
function mjSaveToPersoPack(type, entry){
  ...
  const list = ids.length ? ids.map(id => `<div class="charlib-item" style="cursor:pointer" onclick="_mjDoSaveToPack('${id}')">
      ...
      <div style="font-size:13px;color:var(--text3)">${(_mjCompLib[id].spells||[]).length} sorts · ${(_mjCompLib[id].items||[]).length} objets · ${(_mjCompLib[id].monsters||[]).length} monstres</div></div>
      <span style="color:var(--cp);font-size:13px">+ Ajouter</span></div>`).join('')
