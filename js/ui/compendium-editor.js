...
async function openPackEditor(compId){
  ...
  const sections = CE_TYPES.map(type => {
    ...
    const rows = arr.length ? arr.map((e,i) => `<div style="display:flex;align-items:center;gap:8px;background:var(--surface2);border:1px solid var(--border);border-radius:7px;padding:7px 9px;margin-bottom:5px">
      ...
      </div>`).join('') : `<div style="font-size:12px;color:var(--text3);font-style:italic;padding:4px 0">Aucune entrée.</div>`;
  ...
  openWideModal(`<div class="pt">📚 Éditeur — ${esc(c.name||'Compendium')}</div>
    <div style="font-size:12px;color:var(--text2);margin-bottom:12px">Crée et modifie ton contenu. Les champs marqués <span style="color:#e57373">*</span> sont obligatoires.</div>
...
function mjSaveToPersoPack(type, entry){
  ...
  const list = ids.length ? ids.map(id => `<div class="charlib-item" style="cursor:pointer" onclick="_mjDoSaveToPack('${id}')">
      ...
      <div style="font-size:11px;color:var(--text3)">${(_mjCompLib[id].spells||[]).length} sorts · ${(_mjCompLib[id].items||[]).length} objets · ${(_mjCompLib[id].monsters||[]).length} monstres</div></div>
      <span style="color:var(--cp);font-size:11px">+ Ajouter</span></div>`).join('')
    : `<div style="font-size:12px;color:var(--text3);font-style:italic;padding:6px 0">Aucun compendium perso pour l'instant.</div>`;
  openModal(`<div class="pt">📚 Enregistrer dans un compendium</div>
    <div style="font-size:12px;color:var(--text2);margin-bottom:12px">Choisis un compendium perso, ou crée-en un nouveau :</div>
