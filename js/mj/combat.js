function mjTabCombat(){
  ...
  const combatantRows=sorted.length?sorted.map((c,i)=>{
    ...
    const condHtml=c.conditions&&c.conditions.length?`<div style="margin-top:3px">${c.conditions.map((cd,ci)=>`<span class="status-badge malus" style="cursor:pointer" onclick="mjRemoveCond(${realIdx},${ci})">âš  ${esc(cd)} âœ•</span>`).join('')}</div>`:'';
    const speedHtml=c.speed?`<div style="font-size:10px;color:var(--text3);margin-top:1px">ðŸš¶ ${esc(c.speed)}</div>`:'';
    return`<div class="combat-row${isActive?' active-turn':''}${isDead?' dead':''}">
      ...
      <div style="width:26px;text-align:center;font-size:18px">${c.isPlayer?(c.avatar||'âš”'):'ðŸ‘¾'}</div>
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:600;color:${isActive?'var(--cp)':'var(--text)'}">${esc(c.name)}${isActive?' â—€':''}</div>
        ${speedHtml}${condHtml}
      </div>
      <div style="display:flex;align-items:center;gap:4px">
        ...
        <button class="btn bsm" style="padding:2px 7px;font-size:14px" onclick="mjHpChange(${realIdx},-1)">âˆ’</button>
        <button class="btn bsm" style="padding:2px 7px;font-size:14px" onclick="mjHpChange(${realIdx},1)">+</button>
        <button class="btn bsm" style="font-size:10px;padding:2px 6px" onclick="mjOpenHpModal(${realIdx})">âœ</button>
        <button class="btn bsm" style="font-size:10px;padding:2px 6px" onclick="mjOpenCondModal(${realIdx})">âš </button>
        <button class="btn bsm" style="font-size:10px;padding:2px 6px;border-color:var(--cp);color:var(--cp)" onclick="mjOpenCombatDice(${realIdx})">ðŸŽ²</button>
        <button class="btn bsm" style="font-size:10px;padding:2px 6px;color:#e53935;border-color:#e53935" onclick="mjRemoveCombatant(${realIdx})">âœ•</button>
        ${isActive?`<button class="btn bsm" style="font-size:10px;padding:2px 8px;border-color:#7c3aed;color:#a78bfa;font-weight:600" onclick="mjNextTurn()">â©</button>`:''}
      </div>
  ...
  const logHtml=_mjCombatLog.length?`<div class="pt" style="margin-top:12px">ðŸ“œ Journal de combat</div>
    <div class="mj-log">${[..._mjCombatLog].reverse().map(l=>`<div class="mj-log-entry">${l}</div>`).join('')}</div>`:'';

  return`<div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px">
      <button class="btn bsm bprimary" onclick="mjAddAllToCombat()">âš” Ajouter joueurs</button>
      <button class="btn bsm bprimary" onclick="mjOpenAddMonster()">ðŸ‰ Ajouter monstre</button>
      ${!_mjCombatStarted&&_mjCombatants.length?`<button class="btn bsm" style="border-color:#ffd54f;color:#ffd54f" onclick="mjStartCombat()">ðŸŽ² Lancer l'initiative</button>`:''}
      ${_mjCombatStarted?`<button class="btn bsm" style="border-color:var(--cp);color:var(--cp)" onclick="mjNextTurn()">â–¶ Tour suivant</button>
        <button class="btn bsm" style="border-color:#4caf50;color:#4caf50" onclick="mjEndCombat()">ðŸ Fin du combat</button>
        <div style="display:flex;align-items:center;gap:6px;padding:4px 10px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;font-size:12px">
          ðŸ”„ Round <strong style="color:var(--cp)">${_mjRound}</strong>
          ${currentC?` â€” Tour de <strong style="color:var(--cp)">${esc(currentC.name)}</strong>`:''}
        </div>`:''}
      ${_mjCombatants.length?`<button class="btn bsm" style="border-color:#e53935;color:#e53935" onclick="mjResetCombat()">ðŸ—‘ RÃ©initialiser</button>`:''}
    </div>
    ${combatantRows}
    ${logHtml}
  </div>`;
...
function _extractPlayerCombatData(p){
  ...
  ['mainhand','offhand','ranged'].forEach(slot=>{
    ...
    if(srdW){
      ...
      attacks.push({name:w.name,atkBonus:profB+atkM,dmgDice:dmgParts[0]||'1d6',dmgBonus:atkM,dmgType:dmgParts.slice(1).join(' '),range:isRanged?'Distance':'Corps Ã  corps'});
    }else{
      // Arme personnalisÃ©e : extraire dÃ©s depuis la desc si possible
      const diceMatch=(w.desc||'').match(/(\d+d\d+)/);
      const atkM=isRanged?dexM:forM;
      attacks.push({name:w.name,atkBonus:profB+atkM,dmgDice:diceMatch?diceMatch[1]:'1d6',dmgBonus:atkM,dmgType:'',range:isRanged?'Distance':'Corps Ã  corps'});
    }
  });
...
function mjAddPlayerToCombat(idx){
  ...
  if(!pp)return;
  const p=pp.charData||{};
  if(_mjCombatants.find(c=>c.uid===pp.uid)){showToast('DÃ©jÃ  dans le combat.');return;}
  const mods=(p.abilities||[0,0,0,0,0,0]).map(v=>Math.floor((v-10)/2));
  const {attacks,spells,traits}=_extractPlayerCombatData(p);
  _mjCombatants.push({id:'player_'+pp.uid,name:p.charName||pp.playerName||'Joueur',hp:p.hp||p.hpMax||1,hpMax:p.hpMax||1,ac:p.ac||10,speed:(p.speed!=null?p.speed:9)+'m',initiative:0,dexMod:mods[1]||0,conditions:[],isPlayer:true,avatar:pp.avatar||'âš”',uid:pp.uid,abilities:p.abilities||[10,10,10,10,10,10],attacks,spells,traits});
  _mjCombatLog.push(`âš” ${esc(p.charName||pp.playerName)} ajoutÃ© au combat.`);
  renderMJContent();
}

function mjAddFamiliarToCombat(playerIdx){
  const pp=_mjPlayersData[playerIdx];if(!pp)return;
  const fam=(pp.charData||{}).familiar;if(!fam?.active){showToast('âŒ Ce joueur n\'a pas de familier actif.');return;}
  const famId='familiar_'+pp.uid;
  if(_mjCombatants.find(c=>c.id===famId)){showToast('ðŸ¦‰ DÃ©jÃ  dans le combat.');return;}
  ...
  _mjCombatants.push({
    ...
    initiative:0,dexMod:mods[1]||0,conditions:[],isPlayer:false,avatar:fam.icon||'ðŸ¦‰',
    abilities:fam.ab||[10,10,10,10,10,10],
    attacks:fam.attacks||[],spells:[],
    traits:(fam.traits||[]).map(t=>({name:'Trait',desc:t,uses:0,dice:''})),
  });
  _mjCombatLog.push(`ðŸ¦‰ ${esc(fam.name)} (familier) ajoutÃ© au combat.`);
  renderMJContent();showToast(`ðŸ¦‰ ${fam.name} ajoutÃ© au combat !`);
}
...
function mjFillFromNPC(idx){
  ...
  showToast('âœ“ '+esc(n.name)+' chargÃ© â€” ajustez si besoin.');
}
...
function mjOpenAddMonster(){
  ...
  const npcSection=_mjNPCs.length?`<div style="margin-bottom:12px;padding:8px;background:var(--surface2);border:1px solid var(--border);border-radius:8px">
      <div style="font-size:11px;font-weight:600;color:var(--cp);margin-bottom:6px">ðŸ‘¤ Mes PNJ (${_mjNPCs.length})</div>
      <div style="max-height:90px;overflow-y:auto">${_mjNPCs.map((n,ni)=>`<div class="aci" onclick="mjFillFromNPC(${ni})" style="padding:5px 8px">
        ...
        <div class="ais">PV ${n.hp||'?'} Â· CA ${n.ac||'?'}</div>
      </div>`).join('')}</div>
    </div>`:'';
  const compSection=MONSTERS_DB
    ?`<div style="margin-bottom:12px;padding:8px;background:var(--surface2);border:1px solid var(--border);border-radius:8px">
        <div style="font-size:11px;font-weight:600;color:var(--cp);margin-bottom:6px">ðŸ“š Compendium (${MONSTERS_DB.length} monstres)</div>
    ...
    :`<div style="text-align:center;margin-bottom:10px">
        <button class="btn bsm bprimary" onclick="loadMonstersDB(()=>{closeModal();mjOpenAddMonster()})">ðŸ“š Charger le compendium de monstres</button>
      </div>`;
  openWideModal(`<div class="pt">ðŸ‰ Ajouter au combat</div>
    ...
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px">
      ...
      <div id="mAdd_attacksList"><div style="font-size:11px;color:var(--text3);font-style:italic;padding:4px 0">Aucune attaque dÃ©finie</div></div>
    </div>
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
      ...
      <div id="mAdd_spellsList"><div style="font-size:11px;color:var(--text3);font-style:italic;padding:4px 0">Aucun sort dÃ©fini</div></div>
    </div>
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:16px">
      ...
      <div id="mAdd_traitsList"><div style="font-size:11px;color:var(--text3);font-style:italic;padding:4px 0">Aucun trait dÃ©fini</div></div>
    </div>
    <div style="display:flex;gap:8px">
      <button class="btn" style="flex:1" onclick="closeModal()">Annuler</button>
      <button class="btn bac" style="flex:2" onclick="mjConfirmAddMonster()">âž• Ajouter au combat</button>
    </div>`);
}

function mjFilterMonsters(q){
  const el=document.getElementById('monResults');if(!el||!MONSTERS_DB)return;
  if(!q.trim()){el.innerHTML=MONSTERS_DB.slice(0,15).map((m,i)=>`<div class="charlib-item" style="padding:5px 8px;cursor:pointer" onclick="mjFillMonster(${i})"><div style="flex:1"><div style="font-size:12px;font-weight:600">${esc(m.n)}</div><div style="font-size:11px;color:var(--text3)">CR ${m.cr||'?'} â€” CA ${m.ac||'?'} â€” ${m.hp||'?'} PV â€” ${m.t||''}</div></div><span style="color:var(--cp);font-size:11px">â†‘ Remplir</span></div>`).join('');return;}
  ...
  el.innerHTML=res.length?res.map(({i,m})=>`<div class="charlib-item" style="padding:5px 8px;cursor:pointer" onclick="mjFillMonster(${i})">
    <div style="flex:1">
      ...
      <div style="font-size:11px;color:var(--text3)">CR ${m.cr||'?'} â€” CA ${m.ac||'?'} â€” ${m.hp||'?'} PV â€” ${m.t||''}</div>
    </div>
    <span style="color:var(--cp);font-size:11px">â†‘ Remplir</span>
  </div>`).join(''):'<div style="font-size:12px;color:var(--text3);text-align:center;padding:6px">Aucun rÃ©sultat</div>';
}
...
function mjFillMonster(idx){
  ...
  acts.forEach(act=>{
    if(!act.n)return;
    // Ignorer les rÃ¨gles variantes (Variant: ...)
    if(act.n.startsWith('Variant:'))return;
    if(!act.atk){
      // Action sans jet d'attaque â†’ trait/action spÃ©ciale
  ...
  if(m.res&&m.res.trim())_mjNewMonsterTraits.push({name:'RÃ©sistances',desc:m.res});
  if(m.imm&&m.imm.trim())_mjNewMonsterTraits.push({name:'ImmunitÃ©s aux dÃ©gÃ¢ts',desc:m.imm});
  if(m.ci&&m.ci.trim())_mjNewMonsterTraits.push({name:'ImmunitÃ©s aux conditions',desc:m.ci});
  ...
  if(m.sk&&m.sk.trim())_mjNewMonsterTraits.push({name:'CompÃ©tences',desc:m.sk});
  if(m.spd&&m.spd.trim()&&m.spd!=='walk 30 ft.')_mjNewMonsterTraits.push({name:'Vitesse',desc:_feetToM(m.spd)});
  // Actions spÃ©ciales (actes sans jet d'attaque) â†’ traits
  _specialActions.forEach(n=>{
    const lc=n.toLowerCase();
    const desc=lc.includes('multiattack')||lc.includes('multiattaque')?'Effectue plusieurs attaques par action (voir manuel).':lc.includes('spellcasting')||lc.includes('innate')?'âš  Lanceur de sorts â€” ajoutez les sorts manuellement ci-dessous.':'Action spÃ©ciale (voir manuel).';
    _mjNewMonsterTraits.push({name:n,desc});
  });
  _mjNewMonsterSpells=[];
  mjRenderAttacksList();mjRenderSpellsList();mjRenderTraitsList();
  // Avertissement si le monstre semble Ãªtre un lanceur de sorts
  ...
  const info=(_mjNewMonsterAttacks.length?_mjNewMonsterAttacks.length+' att.':'')+(_mjNewMonsterTraits.length?(_mjNewMonsterAttacks.length?' Â· ':'')+_mjNewMonsterTraits.length+' trait(s)':'');
  const spellWarn=likelyCaster?'<div style="font-size:10px;color:#ffd54f;text-align:center;margin-top:2px">âš  Ce monstre a probablement des sorts â€” Ã  ajouter manuellement.</div>':'';
  if(res)res.innerHTML='<div style="font-size:11px;color:var(--cp);text-align:center;padding:4px">âœ… "'+esc(m.n)+'" importÃ©'+(info?' ('+info+')':'')+'</div>'+spellWarn;
}
...
function mjConfirmAddMonster(){
  ...
  _mjCombatLog.push(`ðŸ‘¾ ${qty>1?qty+'Ã— ':''}"${esc(name)}" ajoutÃ©(s) au combat.`);
  ...
  el.innerHTML=_mjNewMonsterAttacks.length
    ?_mjNewMonsterAttacks.map((a,i)=>`<div style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:rgba(255,255,255,.04);border-radius:6px;margin-bottom:4px">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:11px;color:var(--text3)">${a.atkBonus>=0?'+':''}${a.atkBonus} att. Â· ${esc(a.dmgDice)}${a.dmgBonus?fmt(a.dmgBonus):''} ${esc(a.dmgType)}${a.range?' Â· '+esc(a.range):''}</div>
        </div>
        <button class="btn bsm" style="color:#e53935;padding:1px 6px;flex-shrink:0" onclick="mjRemoveFormAttack(${i})">âœ•</button>
      </div>`).join('')
    :'<div style="font-size:11px;color:var(--text3);font-style:italic;padding:4px 0">Aucune attaque dÃ©finie</div>';
}

function mjConfirmAddFormAttack(){
  const name=(document.getElementById('mAtt_name')?.value||'').trim();
  if(!name){showToast('âŒ Nom de l\'attaque requis.');return;}
...
function mjRenderSpellsList(){
  ...
  el.innerHTML=_mjNewMonsterSpells.length
    ?_mjNewMonsterSpells.map((s,i)=>`<div style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:rgba(255,255,255,.04);border-radius:6px;margin-bottom:4px">
        ...
        <button class="btn bsm" style="color:#e53935;padding:1px 6px;flex-shrink:0" onclick="mjRemoveFormSpell(${i})">âœ•</button>
      </div>`).join('')
    :'<div style="font-size:11px;color:var(--text3);font-style:italic;padding:4px 0">Aucun sort dÃ©fini</div>';
}

function mjConfirmAddFormSpell(){
  const name=(document.getElementById('mSp_name')?.value||'').trim();
  if(!name){showToast('âŒ Nom du sort requis.');return;}
...
let _mjSpellHits=[];
const _DMG_TYPE_FR={'fire damage':'feu','cold damage':'froid','lightning damage':'foudre','thunder damage':'tonnerre','poison damage':'poison','acid damage':'acide','radiant damage':'radiant','necrotic damage':'nÃ©crotique','psychic damage':'psychique','force damage':'force','piercing damage':'perforant','slashing damage':'tranchant','bludgeoning damage':'contondant'};
function _mjSpellSearch(q){
  ...
  if(!db||!db.length){
    ...
    res.innerHTML='<div style="padding:8px;font-size:11px;color:var(--text3)">â³ Chargement du compendium...</div>';
  ...
  if(!_mjSpellHits.length){res.style.display='block';res.innerHTML='<div style="padding:8px;font-size:11px;color:var(--text3);font-style:italic">Aucun sort trouvÃ©.</div>';return;}
  res.style.display='block';
  res.innerHTML=_mjSpellHits.map((s,i)=>`<div onclick="_mjSpellFill(${i})" style="padding:7px 10px;cursor:pointer;border-bottom:1px solid rgba(255,255,255,.06)" onmouseenter="this.style.background='rgba(200,168,75,.1)'" onmouseleave="this.style.background=''"><div style="font-size:12px;font-weight:600;color:var(--text)">${esc(s.name)}</div><div style="font-size:10px;color:var(--text3)">${s.level===0?'Tour de magie':'Niveau '+(s.level||1)} Â· ${esc(s.school||'')}${s.damage?' Â· ðŸŽ² '+s.damage:''}</div></div>`).join('');
}
...
function mjRenderTraitsList(){
  ...
  el.innerHTML=_mjNewMonsterTraits.length
    ?_mjNewMonsterTraits.map((t,i)=>`<div style="display:flex;align-items:flex-start;gap:6px;padding:6px 8px;background:rgba(255,255,255,.04);border-radius:6px;margin-bottom:4px">
        <span style="margin-top:1px">ðŸ“œ</span>
        <div style="flex:1;min-width:0">
          <div style="font-size:12px;font-weight:600">${esc(t.name)}</div>
          ${t.desc?`<div style="font-size:11px;color:var(--text3)">${esc(t.desc)}</div>`:''}
          ${(t.uses&&t.uses>0)||t.dice?`<div style="font-size:10px;color:var(--cp);margin-top:2px">${t.uses>0?t.uses+'Ã— Â· '+(t.recovery==='short'?'Repos court':t.recovery==='long'?'Repos long':'Passif'):''}${t.dice?' Â· ðŸŽ² '+esc(t.dice):''}</div>`:''}
        </div>
        <button class="btn bsm" style="color:#e53935;padding:1px 6px;flex-shrink:0" onclick="mjRemoveFormTrait(${i})">âœ•</button>
      </div>`).join('')
    :'<div style="font-size:11px;color:var(--text3);font-style:italic;padding:4px 0">Aucun trait dÃ©fini</div>';
}

function mjConfirmAddFormTrait(){
  const name=(document.getElementById('mTr_name')?.value||'').trim();
  if(!name){showToast('âŒ Nom du trait requis.');return;}
...
function mjRollTrait(cIdx,tIdx){
  ...
  if(!m){showToast(`ðŸŽ² ${t.name} : formule non reconnue`);return;}
  ...
  if(el){el.style.display='block';el.innerHTML=`<strong>${esc(c.name)}</strong> â€” ${esc(t.name)}<br>ðŸŽ² [${rolls.join('+')}]${m[3]||''} = <span style="font-size:22px;font-weight:700;color:var(--cp)">${total}</span>`;}
  _mjCombatLog.push(`ðŸŽ² ${esc(c.name)} ${esc(t.name)} : ${total}`);
}
function mjUseTraitCharge(cIdx,traitName,maxUses){
  ...
  if(cur<=0){showToast('âŒ Plus de charges disponibles !');return;}
...
function mjGroupRest(type){
  ...
  const label=type==='long'?'ðŸŒ™ Repos long':'â˜• Repos court';
  const detail=type==='long'
    ?'PV remis Ã  fond, sorts, capacitÃ©s et jets de mort rÃ©initialisÃ©s.'
    :'CapacitÃ©s Ã  repos court rÃ©cupÃ©rÃ©es.';
  openModal(`<div class="pt">${label} â€” Groupe</div>
    <p style="font-size:13px;color:var(--text2);margin-bottom:16px">${detail}</p>
    <div style="display:flex;gap:8px">
      <button class="btn bsm bac" style="flex:1" onclick="mjGroupRestConfirm('${type}')">âœ“ Confirmer pour tous</button>
...
async function mjGroupRestConfirm(type){
  ...
  showToast(`${type==='long'?'ðŸŒ™ Repos long':'â˜• Repos court'} appliquÃ© Ã  ${count} joueur(s) !`);
  renderMJContent();
}
...
function mjStartCombat(){
  ...
  _mjCombatLog.push(`ðŸŽ² Initiative lancÃ©e â€” Round 1. Premier : ${esc(sorted[0]?.name||'?')} (${sorted[0]?.initiative||0})`);
...
function mjNextTurn(){
  ...
  _mjCurrentTurn++;
  if(_mjCurrentTurn>=sorted.length){_mjCurrentTurn=0;_mjRound++;_mjCombatLog.push(`ðŸ”„ DÃ©but du Round ${_mjRound}.`);}
  const cur=sorted[_mjCurrentTurn];
  _mjCombatLog.push(`â–¶ Tour de ${esc(cur?.name||'?')}`);
...
function mjEndCombat(){
  ...
  openWideModal(`<div class="pt">ðŸ Fin du combat</div>
    ...
    ${survivors.length?`<div style="margin-bottom:12px">
      <div class="fl mb6" style="color:#4caf50">âœ… Debout (${survivors.length})</div>
      ${survivors.map(c=>`<div style="font-size:12px;padding:5px 10px;background:rgba(76,175,80,.08);border-radius:6px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center"><span>${c.isPlayer?(c.avatar||'âš”')+' ':' ðŸ‘¾ '}${esc(c.name)}</span><span style="color:#4caf50;font-weight:600">${c.hp}/${c.hpMax} PV</span></div>`).join('')}
    </div>`:''}
    ${fallen.length?`<div style="margin-bottom:16px">
      <div class="fl mb6" style="color:#e53935">ðŸ’€ Hors combat (${fallen.length})</div>
      ${fallen.map(c=>`<div style="font-size:12px;padding:5px 10px;background:rgba(229,57,53,.08);border-radius:6px;margin-bottom:4px">${c.isPlayer?(c.avatar||'âš”')+' ':' ðŸ‘¾ '}${esc(c.name)}</div>`).join('')}
    </div>`:''}
    <div style="display:flex;gap:8px;margin-top:8px">
      <button class="btn" style="flex:1" onclick="closeModal()">Annuler</button>
      <button class="btn bac" style="flex:2" onclick="_mjDoEndCombat()">ðŸ Terminer le combat</button>
    </div>`);
}
function _mjDoEndCombat(){
  ...
  closeModal();renderMJContent();showToast('ðŸ Combat terminÃ© !');
}
function mjResetCombat(){
  if(!confirm('RÃ©initialiser le combat ?'))return;
  _mjCombatants=[];_mjCombatStarted=false;_mjCurrentTurn=0;_mjRound=1;_mjCombatLog=[];
  if(currentUser&&currentCampaignId){
    fbDb.collection('characters').doc(currentUser.uid+'_'+currentCampaignId+'_mj').update({combatState:{active:false}}).catch(()=>{});
  }
...
// â”€â”€â”€ JETS DE DÃ‰S PAR COMBATTANT â”€â”€â”€
function mjOpenCombatDice(idx){
  ...
  const attacksHtml=attacks.length
    ?attacks.map((a,ai)=>`<div style="display:flex;align-items:center;gap:8px;padding:8px;background:rgba(255,255,255,.04);border-radius:8px;margin-bottom:6px">
        <div style="flex:1;min-width:0">
          <div style="font-size:13px;font-weight:700">âš” ${esc(a.name)}</div>
          <div style="font-size:11px;color:var(--text3)">${a.atkBonus>=0?'+':''}${a.atkBonus} att. Â· ${esc(a.dmgDice)}${a.dmgBonus?fmt(a.dmgBonus):''} ${esc(a.dmgType||'')}${a.range?' Â· '+esc(a.range):''}</div>
        </div>
        <button class="btn bac bsm" style="flex-shrink:0" onclick="mjRollAttack(${idx},${ai})">ðŸŽ² Lancer</button>
      </div>`).join('')
    :`<div style="text-align:center;padding:20px;color:var(--text3);font-style:italic">Aucune attaque dÃ©finie.<br><span style="font-size:11px">Utilisez âœ Ã‰diter pour en ajouter.</span></div>`;
  const spellsHtml=spells.length
    ?spells.map((s,si)=>`<div style="padding:8px;background:rgba(255,255,255,.04);border-radius:8px;margin-bottom:6px">
        <div style="display:flex;align-items:center;gap:8px">
          <div style="flex:1;min-width:0">
            <div style="font-size:13px;font-weight:700">âœ¦ ${esc(s.name)}${s.level?` <span style="font-size:11px;font-weight:400;color:var(--text3)">Niv.${s.level}</span>`:''}</div>
            <div style="font-size:11px;color:var(--text3)">${s.saveStat&&s.saveDC?`JS ${esc(s.saveStat)} DD <b>${s.saveDC}</b>`:''} ${s.dmgDice?'Â· '+esc(s.dmgDice)+' '+esc(s.dmgType||''):''}</div>
          </div>
          ${s.dmgDice||s.saveDC?`<button class="btn bac bsm" style="flex-shrink:0" onclick="mjRollSpell(${idx},${si})">ðŸŽ² Lancer</button>`:''}
        </div>
        ${s.desc?`<div style="font-size:11px;color:var(--text2);margin-top:4px;font-style:italic">${esc(s.desc)}</div>`:''}
      </div>`).join('')
    :`<div style="text-align:center;padding:20px;color:var(--text3);font-style:italic">Aucun sort dÃ©fini.<br><span style="font-size:11px">Utilisez âœ Ã‰diter pour en ajouter.</span></div>`;
  const traitsHtml=traits.length
    ?traits.map((t,ti)=>{
      ...
      return`<div style="padding:8px;background:rgba(255,255,255,.04);border-radius:8px;margin-bottom:6px">
        <div style="font-size:13px;font-weight:700;margin-bottom:4px">ðŸ“œ ${esc(t.name)}</div>
        ${t.desc?`<div style="font-size:12px;color:var(--text2);margin-bottom:6px">${esc(t.desc)}</div>`:''}
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ${t.dice?`<button class="btn bsm bac" onclick="mjRollTrait(${idx},${ti})">ðŸŽ² ${esc(t.dice)}</button>`:''}
          ${hasUses?`<div style="display:flex;gap:3px">${Array.from({length:maxUses},(_,bi)=>`<span class="slot-bubble${bi<remaining?'':' used'}" onclick="mjUseTraitCharge(${idx},'${esc(t.name)}',${maxUses})"></span>`).join('')}</div><span style="font-size:10px;color:var(--text3)">${remaining}/${maxUses}${recovLabel?' Â· '+recovLabel:''}</span><button class="btn bsm" style="padding:1px 6px" onclick="mjRecoverTraitCharge(${idx},'${esc(t.name)}',${maxUses})">â†º</button>`:''}
        </div>
      </div>`;
    }).join('')
    :`<div style="text-align:center;padding:20px;color:var(--text3);font-style:italic">Aucun trait dÃ©fini.<br><span style="font-size:11px">Utilisez âœ Ã‰diter pour en ajouter.</span></div>`;
  openWideModal(`<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">
    <div class="pt" style="margin:0;flex:1">ðŸŽ² ${esc(c.name)}</div>
    <button class="btn bsm" onclick="mjOpenEditMonster(${idx})">âœ Ã‰diter</button>
  </div>
  <div class="journal-subtab" style="margin-bottom:14px">
    <button class="on" onclick="mjDiceShowTab(this,'dtests')">ðŸŽ² Tests</button>
    <button onclick="mjDiceShowTab(this,'dattacks')">âš” Attaques${attacks.length?' ('+attacks.length+')':''}</button>
    <button onclick="mjDiceShowTab(this,'dspells')">âœ¦ Sorts${spells.length?' ('+spells.length+')':''}</button>
    <button onclick="mjDiceShowTab(this,'dtraits')">ðŸ“œ Traits${traits.length?' ('+traits.length+')':''}</button>
  </div>
  <div id="dtests">
    <div style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">CaractÃ©ristiques</div>
    ...
    <div style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">DÃ©s libres</div>
...
function mjCombatRoll(idx,label,bonus){
  ...
  const el=document.getElementById('mjDiceResult');
  if(el){el.style.display='block';el.innerHTML=`<strong>${esc(c.name)}</strong> â€” ${label}<br>d20(${r}) ${fmt(bonus)} = <span style="font-size:20px;color:${col}">${total}</span>${isCrit?' ðŸŽ‰ CRITIQUE!':isFumble?' ðŸ’€ FUMBLE!':''}`;}
  _mjCombatLog.push(`ðŸŽ² ${esc(c.name)} ${label} : ${r}${bonus?fmt(bonus):''} = <b>${total}</b>${isCrit?' ðŸŽ‰':isFumble?' ðŸ’€':''}`);
}
...
function mjRollAttack(cIdx,aIdx){
  ...
  if(a.dmgDice){
    ...
    dmgHtml=`<div style="font-size:13px;margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">DÃ©gÃ¢ts : <span style="color:var(--text2)">${rolls.join('+')}${dmgBonus?fmt(dmgBonus):''}</span> = <b style="color:var(--cp);font-size:16px">${totalDmg}</b> <span style="font-size:11px;color:var(--text3)">${esc(a.dmgType||'')}${isCrit?' (critique â€” dÃ©s doublÃ©s)':''}</span></div>`;
    logDmg=` / ${totalDmg} dÃ©gÃ¢ts ${a.dmgType||''}${isCrit?' ðŸŽ‰':''}`;
  }
  const el=document.getElementById('mjDiceResult');
  if(el){el.style.display='block';el.innerHTML=`<div style="font-size:12px;color:var(--text3);margin-bottom:4px">${esc(c.name)} â€” ${esc(a.name)}</div><div>d20(${r})${fmt(atkBonus)} = <span style="font-size:22px;color:${col};font-weight:800">${total}</span>${isCrit?' ðŸŽ‰ CRITIQUE!':isFumble?' ðŸ’€ FUMBLE!':''}</div>${dmgHtml}`;}
  _mjCombatLog.push(`âš” ${esc(c.name)} â€” ${esc(a.name)} : att.=${r}${fmt(atkBonus)}=<b>${total}</b>${isCrit?' ðŸŽ‰':isFumble?' ðŸ’€':''}${logDmg}`);
}
...
function mjRollSpell(cIdx,sIdx){
  ...
  if(s.dmgDice){
    ...
    dmgHtml=`<div style="font-size:13px;margin-top:6px">DÃ©gÃ¢ts : <span style="color:var(--text2)">${rolls.join('+')}</span> = <b style="color:var(--cp);font-size:16px">${totalDmg}</b> <span style="font-size:11px;color:var(--text3)">${esc(s.dmgType||'')}</span></div>`;
    logDmg=` / ${totalDmg} dÃ©gÃ¢ts ${s.dmgType||''}`;
  }
  const others=_mjCombatants.filter((_,i)=>i!==cIdx);
  const saveTargetsHtml=s.saveStat&&s.saveDC&&others.length?`<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border)"><div style="font-size:11px;color:var(--text3);margin-bottom:5px">ðŸŽ¯ Lancer le JS ${esc(s.saveStat)} DD${s.saveDC} pour :</div><div style="display:flex;flex-wrap:wrap;gap:4px">${others.map(oc=>`<button class="btn bsm" style="font-size:10px" onclick="mjRollSaveFor(${_mjCombatants.indexOf(oc)},${cIdx},${sIdx})">${esc(oc.name)}</button>`).join('')}</div></div>`:'';
  const el=document.getElementById('mjDiceResult');
  if(el){el.style.display='block';el.innerHTML=`<div style="font-size:12px;color:var(--text3);margin-bottom:4px">${esc(c.name)} â€” ${esc(s.name)}</div>${s.saveStat&&s.saveDC?`<div style="font-size:14px;margin-bottom:4px">JS <b>${esc(s.saveStat)}</b> DD <span style="font-size:22px;color:var(--cp);font-weight:800">${s.saveDC}</span></div>`:''}${dmgHtml||'<div style="color:var(--text3);font-size:12px">Sort d\'effet â€” pas de jets de dÃ©gÃ¢ts</div>'}${saveTargetsHtml}`;}
  _mjCombatLog.push(`âœ¦ ${esc(c.name)} â€” ${esc(s.name)}${s.saveDC?` DD${s.saveDC} ${s.saveStat}`:''}${logDmg}`);
}
...
function mjRollSaveFor(targetIdx,casterIdx,spellIdx){
  ...
  const el=document.getElementById('mjDiceResult');
  if(el){el.innerHTML+=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid var(--border);font-size:12px"><b>${esc(target.name)}</b> JS ${esc(spell.saveStat||'')} : d20(${d20})${mod>=0?'+':''}${mod} = <b style="color:${success?'#4caf50':'#e53935'};font-size:15px">${total}</b> ${success?'âœ… RÃ©ussite':'âŒ Ã‰chec'}</div>`;}
  _mjCombatLog.push(`ðŸŽ¯ JS ${esc(spell.saveStat||'')} DD${spell.saveDC} â€” ${esc(target.name)} : ${total} (${success?'RÃ©ussite':'Ã‰chec'})`);
}
...
function mjOpenEditMonster(idx){
  ...
  openWideModal(`<div class="pt">âœ Ã‰diter â€” ${esc(c.name)}</div>
    <div class="g2" style="gap:8px;margin-bottom:16px">
    ...
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px">
      ...
      <div id="mEdAttForm" style="display:none;background:var(--surface2);border-radius:8px;padding:10px;margin-bottom:8px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:6px">
          ...
          <div><div style="font-size:10px;color:var(--text3);margin-bottom:3px">DÃ©s dÃ©gÃ¢ts</div><input class="fi" id="mAtt_dice" value="1d6"></div>
          <div><div style="font-size:10px;color:var(--text3);margin-bottom:3px">Bonus dÃ©gÃ¢ts</div><input class="fi" id="mAtt_dmgBonus" type="number" value="0"></div>
        </div>
        <div class="g2" style="gap:6px;margin-bottom:8px">
          <div><input class="fi" id="mAtt_type" placeholder="Type dÃ©gÃ¢ts"></div>
          <div><input class="fi" id="mAtt_range" placeholder="PortÃ©e (optionnel)"></div>
        </div>
        <button class="btn bac bsm" style="width:100%" onclick="mjConfirmAddFormAttack()">âœ“ Confirmer</button>
      </div>
    ...
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px">
      ...
      <div id="mEdSpForm" style="display:none;background:var(--surface2);border-radius:8px;padding:10px;margin-bottom:8px">
        ...
        <div class="g2" style="gap:6px;margin-bottom:6px">
          <div><input class="fi" id="mSp_dice" placeholder="DÃ©s (ex: 8d6)"></div>
          <div><input class="fi" id="mSp_type" placeholder="Type dÃ©gÃ¢ts"></div>
        </div>
        <textarea class="fi" id="mSp_desc" rows="2" placeholder="Description..." style="resize:vertical;margin-bottom:8px"></textarea>
        <button class="btn bac bsm" style="width:100%" onclick="mjConfirmAddFormSpell()">âœ“ Confirmer</button>
      </div>
      <div id="mEd_spellsList"></div>
    </div>
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        <div style="font-size:12px;font-weight:700;color:var(--text2)">ðŸ“œ Traits & CapacitÃ©s</div>
      ...
      <div id="mEdTrForm" style="display:none;background:var(--surface2);border-radius:8px;padding:10px;margin-bottom:8px">
        ...
        <button class="btn bac bsm" style="width:100%" onclick="mjConfirmAddFormTrait()">âœ“ Confirmer</button>
      </div>
      <div id="mEd_traitsList"></div>
    </div>
    <div style="display:flex;gap:8px">
      <button class="btn" style="flex:1" onclick="closeModal()">Annuler</button>
      <button class="btn bac" style="flex:2" onclick="mjSaveEditMonster()">ðŸ’¾ Sauvegarder</button>
    </div>`);
  mjRenderAttacksList();mjRenderSpellsList();mjRenderTraitsList();
}
...
function mjSaveEditMonster(){
  ...
  _mjNewMonsterAttacks=[];_mjNewMonsterSpells=[];_mjNewMonsterTraits=[];_mjEditingMonsterIdx=-1;
  _mjCombatLog.push(`âœ ${esc(c.name)} modifiÃ©.`);
  closeModal();renderMJContent();
}
function mjCombatRollFree(idx,d){
  ...
  const el=document.getElementById('mjDiceResult');
  if(el){el.style.display='block';el.innerHTML=`<strong>${esc(c.name)}</strong> â€” ${d}<br><span style="font-size:20px">${r}</span>${isCrit?' ðŸŽ‰':isFumble?' ðŸ’€':''}`;}
  _mjCombatLog.push(`ðŸŽ² ${esc(c.name)} ${d} : <b>${r}</b>${isCrit?' ðŸŽ‰':isFumble?' ðŸ’€':''}`);
}
...
function mjRemoveCombatant(idx){
  ...
  _mjCombatLog.push(`âœ• ${esc(name)} retirÃ© du combat.`);
...
function _mjSyncPlayerHp(c){
  ...
  _mjHpSyncTimers[c.uid]=setTimeout(async()=>{
    try{
      ...
      if(inWs){
        // Le joueur est en forme animale â€” syncer les PV de la bÃªte
        ...
        if(beastHp<=0){
          // La bÃªte tombe â€” dÃ©clencher le retour Ã  la forme druide
...
function mjHpChange(idx,dir){
  ...
  if(c.hp===0)_mjCombatLog.push(`ðŸ’€ ${esc(c.name)} tombe Ã  0 PV !`);
  _mjSyncPlayerHp(c);
  renderMJContent();
}
...
function mjOpenHpModal(idx){
  const c=_mjCombatants[idx];if(!c)return;
  openModal(`<div class="pt">â¤ PV â€” ${esc(c.name)}</div>
    ...
    <div class="g2" style="gap:8px;margin-bottom:16px">
      ...
      <div><div class="fl mb6">DÃ©gÃ¢ts (âˆ’PV)</div><input class="fi" id="hpModal_dmg" type="number" min="0" placeholder="0"></div>
    </div>
    <div><div class="fl mb6">DÃ©finir PV max</div><input class="fi" id="hpModal_max" type="number" min="1" value="${c.hpMax}" style="margin-bottom:16px"></div>
    <div style="display:flex;gap:8px">
      <button class="btn" style="flex:1" onclick="closeModal()">Annuler</button>
...
function mjApplyHpModal(idx){
  ...
  if(heal) _mjCombatLog.push(`ðŸ’š ${esc(c.name)} soignÃ© de ${heal} PV (â†’ ${c.hp}/${c.hpMax})`);
  if(dmg) _mjCombatLog.push(`ðŸ—¡ ${esc(c.name)} subit ${dmg} dÃ©gÃ¢ts (â†’ ${c.hp}/${c.hpMax})`);
  if(c.hp===0)_mjCombatLog.push(`ðŸ’€ ${esc(c.name)} tombe Ã  0 PV !`);
  _mjSyncPlayerHp(c);
  closeModal();renderMJContent();
}
...
const MJ_CONDITIONS=[
  {n:'AveuglÃ©',d:'Ne peut pas voir. Attaques contre lui : avantage. Ses attaques : dÃ©savantage.'},
  {n:'CharmÃ©',d:'Ne peut pas attaquer le charmeur. Le charmeur a avantage aux interactions sociales.'},
  {n:'Assourdi',d:'Ne peut pas entendre. Ã‰choue automatiquement les JS liÃ©s Ã  l\'ouÃ¯e.'},
  {n:'Ã‰pouvantÃ©',d:'DÃ©savantage aux jets d\'attaque et caractÃ©ristiques si la source de peur est visible.'},
  {n:'AgrippÃ©',d:'Vitesse = 0. Condition retirÃ©e si hors de portÃ©e ou si incapacitÃ©.'},
  {n:'NeutralisÃ©',d:'Ne peut pas effectuer d\'actions ni de rÃ©actions.'},
  {n:'Invisible',d:'Impossible Ã  voir sans magie. Attaques contre lui : dÃ©savantage. Ses attaques : avantage.'},
  {n:'ParalysÃ©',d:'NeutralisÃ©, ne peut bouger/parler. Attaques au corps Ã  corps = coup critique automatique Ã  moins de 1,5m.'},
  {n:'PÃ©trifiÃ©',d:'TransformÃ© en pierre. NeutralisÃ©, rÃ©sistance Ã  tous les dÃ©gÃ¢ts, immunitÃ© poison/maladie.'},
  {n:'EmpoisonnÃ©',d:'DÃ©savantage aux jets d\'attaque et de caractÃ©ristique.'},
  {n:'Ã€ terre',d:'Se relever coÃ»te la moitiÃ© de la vitesse. Attaques au corps Ã  corps contre lui : avantage. Ã€ distance : dÃ©savantage.'},
  {n:'EntravÃ©',d:'Vitesse = 0. Attaques contre lui : avantage. Ses attaques : dÃ©savantage. JS DEX : dÃ©savantage.'},
  {n:'Ã‰tourdi',d:'NeutralisÃ©, ne peut bouger. Attaques contre lui : avantage. Ã‰choue automatiquement JS FOR/DEX.'},
  {n:'Inconscient',d:'NeutralisÃ©, tombe Ã  terre, lÃ¢che tout. Attaques contre lui : avantage. Coup critique au corps Ã  corps < 1,5m.'},
  {n:'Ã‰puisÃ©',d:'Malus selon le niveau (1â†’dÃ©savantage jets compÃ©tences, 3â†’vitesse divisÃ©e par 2, 5â†’vitesse 0, 6â†’mort).'},
  {n:'Concentration',d:'Le lanceur est en concentration. Un coup peut rompre la concentration (JS CON DD 10 ou moitiÃ© des dÃ©gÃ¢ts).'},
];

function mjEditInitiative(idx){
  const c=_mjCombatants[idx];if(!c)return;
  openModal(`<div class="pt">ðŸŽ² Initiative â€” ${esc(c.name)}</div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
      <div style="flex:1"><div class="fl mb6">Valeur (dÃ© + mod)</div><input class="fi" id="initInput" type="number" value="${c.initiative||0}" style="font-size:20px;text-align:center"></div>
      <button class="btn bsm" style="margin-top:18px;border-color:#ffd54f;color:#ffd54f" onclick="mjRerollInitiative(${idx})">ðŸŽ² Relancer</button>
    </div>
    <div style="display:flex;gap:8px">
      <button class="btn" style="flex:1" onclick="closeModal()">Annuler</button>
      <button class="btn bac" style="flex:2" onclick="_mjConfirmInitiative(${idx})">âœ“ Confirmer</button>
    </div>`);
...
function mjRerollInitiative(idx){
  ...
  _mjCombatLog.push(`ðŸŽ² ${esc(c.name)} relance l'initiative : ${c.initiative}`);
  closeModal();renderMJContent();
}
...
function mjOpenCondModal(idx){
  ...
  openWideModal(`<div class="pt">âš  Conditions â€” ${esc(c.name)}</div>
    ...
    <div class="fl mb6">Condition personnalisÃ©e</div>
    <div style="display:flex;gap:6px;margin-bottom:14px">
      <input class="fi" id="condCustom" placeholder="Ex: Maudit, BÃ©ni, Rage..." style="flex:1">
...
function mjAddCustomCond(idx){
  ...
  showToast(`âœ… Condition "${val}" ajoutÃ©e.`);
}
