function mjTabCombat(){
  ...
  const combatantRows=sorted.length?sorted.map((c,i)=>{
    ...
    const speedHtml=c.speed?`<div style="font-size:12px;color:var(--text3);margin-top:1px">🚶 ${esc(c.speed)}</div>`:'';
...
function mjOpenAddMonster(){
  ...
  openWideModal(`<div class="pt">🐉 Ajouter au combat</div>
    ...
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px">
      ...
      <div id="mAdd_attacksList"><div style="font-size:13px;color:var(--text3);font-style:italic;padding:4px 0">Aucune attaque définie</div></div>
    </div>
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
      ...
      <div id="mAdd_spellsList"><div style="font-size:13px;color:var(--text3);font-style:italic;padding:4px 0">Aucun sort défini</div></div>
    </div>
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:16px">
      ...
      <div id="mAdd_traitsList"><div style="font-size:13px;color:var(--text3);font-style:italic;padding:4px 0">Aucun trait défini</div></div>
    </div>
...
function mjFilterMonsters(q){
  ...
  if(!q.trim()){
    ...
    el.innerHTML=sorted.map(({i,m})=>`<div class="charlib-item" style="padding:5px 8px;cursor:pointer" onclick="mjFillMonster(${i})"><div style="flex:1"><div style="font-size:13px;font-weight:600">${esc(m.n)}</div><div style="font-size:13px;color:var(--text3)">CR ${m.cr||'?'} — CA ${m.ac||'?'} — ${m.hp||'?'} PV — ${m.t||''}</div></div><span style="color:var(--cp);font-size:13px">↑ Remplir</span></div>`).join('');return;}
  ...
  el.innerHTML=res20.length?res20.map(({i,m})=>`<div class="charlib-item" style="padding:5px 8px;cursor:pointer" onclick="mjFillMonster(${i})">
    <div style="flex:1">
      ...
      <div style="font-size:13px;color:var(--text3)">CR ${m.cr||'?'} — CA ${m.ac||'?'} — ${m.hp||'?'} PV — ${m.t||''}</div>
    </div>
    <span style="color:var(--cp);font-size:13px">↑ Remplir</span>
...
function mjFillMonster(idx){
  ...
  const spellWarn=likelyCaster?'<div style="font-size:12px;color:var(--cp);text-align:center;margin-top:2px">⚠ Ce monstre a probablement des sorts — à ajouter manuellement.</div>':'';
  if(res)res.innerHTML='<div style="font-size:13px;color:var(--cp);text-align:center;padding:4px">✅ "'+esc(m.n)+'" importé'+(info?' ('+info+')':'')+'</div>'+spellWarn;
}
...
function mjRenderAttacksList(){
  ...
  el.innerHTML=_mjNewMonsterAttacks.length
    ?_mjNewMonsterAttacks.map((a,i)=>`<div style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:rgba(255,255,255,.04);border-radius:2px;margin-bottom:4px">
        <span>⚔</span>
        <div style="flex:1;min-width:0">
        ...
        <button class="btn bsm" style="color:var(--danger);padding:1px 6px;flex-shrink:0" onclick="mjRemoveFormAttack(${i})">✕</button>
      </div>`).join('')
    :'<div style="font-size:13px;color:var(--text3);font-style:italic;padding:4px 0">Aucune attaque définie</div>';
}
...
function mjRenderSpellsList(){
  ...
  el.innerHTML=_mjNewMonsterSpells.length
    ?_mjNewMonsterSpells.map((s,i)=>`<div style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:rgba(255,255,255,.04);border-radius:2px;margin-bottom:4px">
        <span>✦</span>
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:13px;color:var(--text3)">${s.saveStat&&s.saveDC?`DD${s.saveDC} ${esc(s.saveStat)} `:''} ${s.dmgDice?esc(s.dmgDice)+' '+esc(s.dmgType):''}</div>
        </div>
        <button class="btn bsm" style="color:var(--danger);padding:1px 6px;flex-shrink:0" onclick="mjRemoveFormSpell(${i})">✕</button>
      </div>`).join('')
    :'<div style="font-size:13px;color:var(--text3);font-style:italic;padding:4px 0">Aucun sort défini</div>';
}
...
function _mjSpellSearch(q){
  ...
  if(!db||!db.length){
    ...
    res.innerHTML='<div style="padding:8px;font-size:13px;color:var(--text3)">⏳ Chargement du compendium...</div>';
  ...
  if(!_mjSpellHits.length){res.style.display='block';res.innerHTML='<div style="padding:8px;font-size:13px;color:var(--text3);font-style:italic">Aucun sort trouvé.</div>';return;}
  res.style.display='block';
  res.innerHTML=_mjSpellHits.map((s,i)=>`<div onclick="_mjSpellFill(${i})" style="padding:7px 10px;cursor:pointer;border-bottom:1px solid rgba(255,255,255,.06)" onmouseenter="this.style.background='rgba(200,168,75,.1)'" onmouseleave="this.style.background=''"><div style="font-size:13px;font-weight:600;color:var(--text)">${esc(s.name)}</div><div style="font-size:12px;color:var(--text3)">${s.level===0?'Tour de magie':'Niveau '+(s.level||1)} · ${esc(s.school||'')}${s.damage?' · 🎲 '+s.damage:''}</div></div>`).join('');
}
...
function mjRenderTraitsList(){
  ...
  el.innerHTML=_mjNewMonsterTraits.length
    ?_mjNewMonsterTraits.map((t,i)=>`<div style="display:flex;align-items:flex-start;gap:6px;padding:6px 8px;background:rgba(255,255,255,.04);border-radius:2px;margin-bottom:4px">
        <span style="margin-top:1px">📜</span>
        <div style="flex:1;min-width:0">
          <div style="font-size:13px;font-weight:600">${esc(t.name)}</div>
          ${t.desc?`<div style="font-size:13px;color:var(--text3)">${esc(t.desc)}</div>`:''}
          ${(t.uses&&t.uses>0)||t.dice?`<div style="font-size:12px;color:var(--cp);margin-top:2px">${t.uses>0?t.uses+'× · '+(t.recovery==='short'?'Repos court':t.recovery==='long'?'Repos long':'Passif'):''}${t.dice?' · 🎲 '+esc(t.dice):''}</div>`:''}
        </div>
        <button class="btn bsm" style="color:var(--danger);padding:1px 6px;flex-shrink:0" onclick="mjRemoveFormTrait(${i})">✕</button>
      </div>`).join('')
    :'<div style="font-size:13px;color:var(--text3);font-style:italic;padding:4px 0">Aucun trait défini</div>';
}
...
function mjRollTrait(cIdx,tIdx){
  ...
  if(el){el.style.display='block';el.innerHTML=`<strong>${esc(c.name)}</strong> — ${esc(t.name)}<br>🎲 [${rolls.join('+')}]${m[3]||''} = <span style="font-size:16px;font-weight:700;color:var(--cp)">${total}</span>`;}
...
function mjEndCombat(){
  ...
  openWideModal(`<div class="pt">🏁 Fin du combat</div>
    ...
    ${survivors.length?`<div style="margin-bottom:12px">
      <div class="fl mb6" style="color:var(--good)">✅ Debout (${survivors.length})</div>
      ${survivors.map(c=>`<div style="font-size:13px;padding:5px 10px;background:rgba(76,175,80,.08);border-radius:2px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center"><span>${c.isPlayer?(c.avatar||'⚔')+' ':' 👾 '}${esc(c.name)}</span><span style="color:var(--good);font-weight:600">${c.hp}/${c.hpMax} PV</span></div>`).join('')}
    </div>`:''}
    ${fallen.length?`<div style="margin-bottom:16px">
      <div class="fl mb6" style="color:var(--danger)">💀 Hors combat (${fallen.length})</div>
      ${fallen.map(c=>`<div style="font-size:13px;padding:5px 10px;background:rgba(229,57,53,.08);border-radius:2px;margin-bottom:4px">${c.isPlayer?(c.avatar||'⚔')+' ':' 👾 '}${esc(c.name)}</div>`).join('')}
    </div>`:''}
...
function mjOpenCombatDice(idx){
  ...
  const makeBtn=(label,bonus,fn)=>`<button class="btn" style="${btnStyle}" onclick="${fn}(${idx},'${label}',${bonus})"><div style="font-size:13px;font-weight:700">${label}</div><div style="font-size:13px;color:var(--cp)">${fmt(bonus)}</div></button>`;
  const attacks=c.attacks||[];const spells=c.spells||[];const traits=c.traits||[];
  const attacksHtml=attacks.length
    ?attacks.map((a,ai)=>`<div style="display:flex;align-items:center;gap:8px;padding:8px;background:rgba(255,255,255,.04);border-radius:2px;margin-bottom:6px">
        <div style="flex:1;min-width:0">
  ...
  const spellsHtml=spells.length
    ?spells.map((s,si)=>`<div style="padding:8px;background:rgba(255,255,255,.04);border-radius:2px;margin-bottom:6px">
    ...
    :`<div style="text-align:center;padding:20px;color:var(--text3);font-style:italic">Aucun sort défini.<br><span style="font-size:13px">Utilisez ✏ Éditer pour en ajouter.</span></div>`;
  const traitsHtml=traits.length
    ?traits.map((t,ti)=>{
      ...
      return`<div style="padding:8px;background:rgba(255,255,255,.04);border-radius:2px;margin-bottom:6px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ...
          ${hasUses?`<div style="display:flex;gap:3px">${Array.from({length:maxUses},(_,bi)=>`<span class="slot-bubble${bi<remaining?'':' used'}" onclick="mjUseTraitCharge(${idx},'${jsq(t.name)}',${maxUses})"></span>`).join('')}</div><span style="font-size:12px;color:var(--text3)">${remaining}/${maxUses}${recovLabel?' · '+recovLabel:''}</span><button class="btn bsm" style="padding:1px 6px" onclick="mjRecoverTraitCharge(${idx},'${jsq(t.name)}',${maxUses})">↺</button>`:''}
        </div>
      </div>`;
    }).join('')
    :`<div style="text-align:center;padding:20px;color:var(--text3);font-style:italic">Aucun trait défini.<br><span style="font-size:13px">Utilisez ✏ Éditer pour en ajouter.</span></div>`;
  ...
  <div id="dtests">
    <div style="font-size:13px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Caractéristiques</div>
    ...
    <div style="font-size:13px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Jets de sauvegarde</div>
    <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:4px;margin-bottom:12px">
      ${AB.map((a,i)=>makeBtn('JS '+a,mods[i],'mjCombatRoll')).join('')}
    </div>
    <div style="font-size:13px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Dés libres</div>
  ...
  <div id="mjDiceResult" style="display:none;padding:10px;background:var(--surface2);border-radius:2px;margin-top:10px;margin-bottom:6px"></div>
...
function mjCombatRoll(idx,label,bonus){
  ...
  const el=document.getElementById('mjDiceResult');
  if(el){el.style.display='block';el.innerHTML=`<strong>${esc(c.name)}</strong> — ${label}<br>d20(${r}) ${fmt(bonus)} = <span style="font-size:15px;color:${col}">${total}</span>${isCrit?' 🎉 CRITIQUE!':isFumble?' 💀 FUMBLE!':''}`;}
...
function mjRollSpell(cIdx,sIdx){
  ...
  if(s.dmgDice){
    ...
    dmgHtml=`<div style="font-size:13px;margin-top:6px">Dégâts : <span style="color:var(--text2)">${rolls.join('+')}</span> = <b style="color:var(--cp);font-size:16px">${totalDmg}</b> <span style="font-size:13px;color:var(--text3)">${esc(s.dmgType||'')}</span></div>`;
    logDmg=` / ${totalDmg} dégâts ${s.dmgType||''}`;
  }
  const others=_mjCombatants.filter((_,i)=>i!==cIdx);
  const saveTargetsHtml=s.saveStat&&s.saveDC&&others.length?`<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border)"><div style="font-size:13px;color:var(--text3);margin-bottom:5px">🎯 Lancer le JS ${esc(s.saveStat)} DD${s.saveDC} pour :</div><div style="display:flex;flex-wrap:wrap;gap:4px">${others.map(oc=>`<button class="btn bsm" style="font-size:12px" onclick="mjRollSaveFor(${_mjCombatants.indexOf(oc)},${cIdx},${sIdx})">${esc(oc.name)}</button>`).join('')}</div></div>`:'';
  const el=document.getElementById('mjDiceResult');
  if(el){el.style.display='block';el.innerHTML=`<div style="font-size:13px;color:var(--text3);margin-bottom:4px">${esc(c.name)} — ${esc(s.name)}</div>${s.saveStat&&s.saveDC?`<div style="font-size:14px;margin-bottom:4px">JS <b>${esc(s.saveStat)}</b> DD <span style="font-size:16px;color:var(--cp);font-weight:800">${s.saveDC}</span></div>`:''}${dmgHtml||'<div style="color:var(--text3);font-size:13px">Sort d\'effet — pas de jets de dégâts</div>'}${saveTargetsHtml}`;}
...
function mjRollSaveFor(targetIdx,casterIdx,spellIdx){
  ...
  const el=document.getElementById('mjDiceResult');
  if(el){el.innerHTML+=`<div style="margin-top:6px;padding-top:6px;border-top:1px solid var(--border);font-size:13px"><b>${esc(target.name)}</b> JS ${esc(spell.saveStat||'')} : d20(${d20})${mod>=0?'+':''}${mod} = <b style="color:${success?'var(--good)':'var(--danger)'};font-size:15px">${total}</b> ${success?'✅ Réussite':'❌ Échec'}</div>`;}
...
function mjOpenEditMonster(idx){
  ...
  openWideModal(`<div class="pt">✏ Éditer — ${esc(c.name)}</div>
    ...
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px">
      ...
      <div id="mEdAttForm" style="display:none;background:var(--surface2);border-radius:2px;padding:10px;margin-bottom:8px">
        <input class="fi" id="mAtt_name" placeholder="Nom (ex: Cimeterre)" style="margin-bottom:6px">
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:6px">
          <div><div style="font-size:12px;color:var(--text3);margin-bottom:3px">Bonus att.</div><input class="fi" id="mAtt_bonus" type="number" value="0"></div>
          <div><div style="font-size:12px;color:var(--text3);margin-bottom:3px">Dés dégâts</div><input class="fi" id="mAtt_dice" value="1d6"></div>
          <div><div style="font-size:12px;color:var(--text3);margin-bottom:3px">Bonus dégâts</div><input class="fi" id="mAtt_dmgBonus" type="number" value="0"></div>
        </div>
        <div class="g2" style="gap:6px;margin-bottom:8px">
    ...
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:12px">
      ...
      <div id="mEdSpForm" style="display:none;background:var(--surface2);border-radius:2px;padding:10px;margin-bottom:8px">
        <div style="position:relative;margin-bottom:8px">
          <input class="fi" id="mjSpSearchQ" placeholder="🔍 Chercher dans le compendium..." oninput="_mjSpellSearch(this.value)" onfocus="_mjSpellSearch(this.value)" autocomplete="off" onblur="setTimeout(()=>{const r=document.getElementById('mjSpSearchRes');if(r)r.style.display='none';},150)">
          <div id="mjSpSearchRes" style="display:none;position:absolute;top:100%;left:0;right:0;z-index:200;background:var(--surface);border:1px solid rgba(200,168,75,.4);border-radius:0 0 6px 6px;max-height:200px;overflow-y:auto;box-shadow:0 4px 16px rgba(0,0,0,.5)"></div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:6px">
          <div><div style="font-size:12px;color:var(--text3);margin-bottom:3px">Niveau</div><input class="fi" id="mSp_level" type="number" value="1" min="0"></div>
          <div><div style="font-size:12px;color:var(--text3);margin-bottom:3px">Stat save</div><input class="fi" id="mSp_saveStat" placeholder="DEX"></div>
          <div><div style="font-size:12px;color:var(--text3);margin-bottom:3px">DD save</div><input class="fi" id="mSp_saveDC" type="number" value="13"></div>
        </div>
        <div class="g2" style="gap:6px;margin-bottom:6px">
    ...
    <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:16px">
      ...
      <div id="mEdTrForm" style="display:none;background:var(--surface2);border-radius:2px;padding:10px;margin-bottom:8px">
...
function mjCombatRollFree(idx,d){
  ...
  const el=document.getElementById('mjDiceResult');
  if(el){el.style.display='block';el.innerHTML=`<strong>${esc(c.name)}</strong> — ${d}<br><span style="font-size:15px">${r}</span>${isCrit?' 🎉':isFumble?' 💀':''}`;}
...
function mjOpenHpModal(idx){
  ...
  openModal(`<div class="pt">❤ PV — ${esc(c.name)}</div>
    <div style="text-align:center;font-size:16px;font-weight:700;color:var(--cp);margin-bottom:12px">${c.hp} / ${c.hpMax}</div>
    <div class="g2" style="gap:8px;margin-bottom:16px">
...
function mjOpenCondModal(idx){
  ...
  openWideModal(`<div class="pt">⚠ Conditions — ${esc(c.name)}</div>
    ...
    <div id="condDesc" style="min-height:28px;font-size:13px;color:var(--text3);font-style:italic;padding:4px 2px;margin-bottom:10px">Survolez une condition pour voir sa description.</div>
