function mjTabPNJ(){
  ...
  const list=_mjNPCs.length?sortedNPCs.map(({n,i})=>`<div class="pnj-card${i===sel?' pnj-selected':''}" onclick="mjSelectNPC(${i})">
    <div style="display:flex;align-items:center;justify-content:space-between">
      <div>
        ...
        <div style="font-size:12px;color:var(--text3)">${esc(n.type||'PNJ')} ${n.cr?'— CR '+n.cr:''} — PV ${n.hp||0} — CA ${n.ac||0}</div>
      </div>
  ...
  </div>`).join(''):`<div style="color:var(--text3);font-size:13px;font-style:italic;padding:8px 0">Aucun PNJ sauvegardé.</div>`;
...
function mjNPCDetail(n,i){
  ...
  const attHtml=attacks.length?`<div style="margin-bottom:8px"><div class="fl mb6">⚔ Attaques</div>${attacks.map(a=>`<div style="font-size:13px;padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px"><span style="font-weight:600">${esc(a.name)}</span><span style="color:var(--text3)"> · +${a.atkBonus||0} · ${a.dmgDice||'1d6'}${a.dmgBonus?'+'+(a.dmgBonus):''}${a.dmgType?' '+esc(a.dmgType):''}${a.range?' · '+esc(a.range):''}</span></div>`).join('')}</div>`:'';
  const spHtml=spells.length?`<div style="margin-bottom:8px"><div class="fl mb6">✦ Sorts & Pouvoirs</div>${spells.map(s=>`<div style="font-size:13px;padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px"><span style="font-weight:600">${esc(s.name)}</span>${s.level?` <span style="color:var(--text3)">Niv.${s.level}</span>`:''}${s.saveStat&&s.saveDC?` <span style="color:var(--text3)">· JS ${esc(s.saveStat)} DD${s.saveDC}</span>`:''}${s.dmgDice?` <span style="color:var(--text3)">· ${esc(s.dmgDice)}${s.dmgType?' '+esc(s.dmgType):''}</span>`:''}${s.desc?`<div style="color:var(--text2);font-size:12px;margin-top:2px">${esc(s.desc)}</div>`:''}</div>`).join('')}</div>`:'';
  const trHtml=traits.length?`<div style="margin-bottom:8px"><div class="fl mb6">📜 Traits & Capacités</div>${traits.map(t=>`<div style="font-size:13px;padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px"><div style="font-weight:600">${esc(t.name)}</div>${t.desc?`<div style="color:var(--text2);font-size:12px;margin-top:2px">${esc(t.desc)}</div>`:''}</div>`).join('')}</div>`:'';
  const oldAttHtml=typeof n.attacks==='string'&&n.attacks?`<div style="margin-bottom:8px"><div class="fl mb6">Attaques</div><div style="font-size:13px;color:var(--text2);background:var(--surface2);border-radius:6px;padding:8px;white-space:pre-wrap">${esc(n.attacks)}</div></div>`:'';
  return`<div class="panel">
    <div class="pt">${esc(n.name||'?')} <span style="font-size:12px;color:var(--text3);font-family:var(--B)">${esc(n.type||'PNJ')}</span></div>
    ...
    ${n.notes?`<div style="margin-bottom:10px"><div class="fl mb6">Notes</div><div style="font-size:13px;color:var(--text2);white-space:pre-wrap;background:var(--surface2);border-radius:6px;padding:8px">${esc(n.notes)}</div></div>`:''}
...
function mjOpenNPCForm(editIdx){
  ...
  openWideModal(`<div class="pt">${title}</div>
    ...
    <div id="pnj_cr_preview" style="font-size:12px;color:var(--text3);text-align:right;margin-bottom:8px;padding-right:2px"></div>
    <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:6px;margin-bottom:16px">
      ${statLabels.map((l,idx)=>`<div>
        <div style="font-size:11px;color:var(--text3);text-align:center;margin-bottom:3px">${l}</div>
...
function mjOpenSidekickForm(){
  openModal(`<div class="pt">🤝 Créer un comparse</div>
    ...
    <div id="sk_preview" style="background:var(--surface2);border-radius:8px;padding:10px;margin-bottom:16px;font-size:13px;color:var(--text2);min-height:60px"></div>
    <div style="display:flex;gap:8px">
      <button class="btn" style="flex:1" onclick="closeModal()">Annuler</button>
...
function mjUpdateSidekickPreview(){
  ...
  prev.innerHTML=`<div style="font-weight:600;color:var(--cp);margin-bottom:6px">${sk.label||'Incantateur'} — Niveau ${lv} · PV max <strong>${hp}</strong> (${hd})</div>
    <ul style="margin:0;padding-left:14px;font-size:12px;line-height:1.7">${abilities.slice(0,lv).map(a=>`<li>${a}</li>`).join('')}</ul>`;
}
