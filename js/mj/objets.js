function mjTabObjets(){
  const itemList=_mjObjets.length?_mjObjets.map((obj,i)=>{
    ...
    <div style="flex:1;min-width:0">
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
        ...
        ${ri?`<span style="font-size:12px;padding:1px 6px;border-radius:2px;background:${ri.color}22;color:${ri.color};border:1px solid ${ri.color}55">${obj.rarity}</span>`:''}
        ${obj.attunement?`<span style="font-size:12px;padding:1px 6px;border-radius:2px;background:rgba(156,39,176,.1);color:var(--arcane);border:1px solid rgba(156,39,176,.3)">🔗 Lien requis</span>`:''}
      </div>
      <div style="font-size:13px;color:var(--text3)">${esc(obj.type||'')}${ri?' · '+ri.level+' · '+ri.price:obj.value?' — '+obj.value+' po':''}</div>
      ${obj.desc?`<div style="font-size:13px;color:var(--text2);margin-top:2px">${esc(obj.desc)}</div>`:''}
    </div>
    <div style="display:flex;gap:4px;align-items:center">
      ...
      <button class="btn bsm" style="color:var(--danger);border-color:var(--danger)" onclick="mjDeleteItem(${i})">✕</button>
    </div>
  ...
  const rarFilterBar=['','Commun','Peu commun','Rare','Très rare','Légendaire','Artefact'].map(r=>{
    ...
    return`<button class="btn bsm" onclick="_encRarityFilter='${r}';renderMJContent();setTimeout(()=>mjFilterItems(document.getElementById('itemSearch')?document.getElementById('itemSearch').value:''),30)" style="font-size:12px;padding:2px 8px;${active?`background:${col};color:#fff;border-color:transparent`:`border-color:var(--border);color:var(--text2)`}">${r||'Tout'}</button>`;
  }).join('');
...
function _mjRarBadge(rar){
  ...
  return`<span style="font-size:12px;padding:1px 6px;border-radius:2px;background:${ri.color}22;color:${ri.color};border:1px solid ${ri.color}55;white-space:nowrap">${rar}</span>`;
}
    ...
    return`<div class="charlib-item" style="padding:6px 8px;cursor:pointer;flex-direction:column;align-items:flex-start;gap:2px" onclick="mjAddItemFromCompendium(${i})">
      <div style="display:flex;align-items:center;gap:6px;width:100%">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:13px;color:var(--text3)">${esc(it.d||'')}${it.mg?' ✨':''}</div>
        </div>
        <span style="color:var(--cp);font-size:13px;white-space:nowrap;flex-shrink:0">+ Ajouter</span>
      </div>
      ${rar?`<div style="display:flex;gap:6px;align-items:center;padding-left:22px">${_mjRarBadge(rar)}${ri?`<span style="font-size:12px;color:var(--text3)">${ri.level} · ${ri.price}</span>`:''}</div>`:''}
    </div>`;
  ...
  openModal(`<div class="pt">🎁 Donner "${esc(item.name||'?')}"</div>
    ...
    ${_mjPlayersData.map((pp,pi)=>`<div class="charlib-item" onclick="mjGiveItem(${itemIdx},${pi})">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:13px;color:var(--text3)">${esc(pp.playerName||'')}</div>
      </div>
