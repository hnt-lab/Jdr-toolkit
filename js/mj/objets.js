function mjTabObjets(){
  const itemList=_mjObjets.length?_mjObjets.map((obj,i)=>{
    ...
    <div style="flex:1;min-width:0">
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
        ...
        ${obj.attunement?`<span style="font-size:10px;padding:1px 6px;border-radius:4px;background:rgba(156,39,176,.1);color:#9c27b0;border:1px solid rgba(156,39,176,.3)">ðŸ”— Lien requis</span>`:''}
      </div>
      <div style="font-size:11px;color:var(--text3)">${esc(obj.type||'')}${ri?' Â· '+ri.level+' Â· '+ri.price:obj.value?' â€” '+obj.value+' po':''}</div>
      ${obj.desc?`<div style="font-size:11px;color:var(--text2);margin-top:2px">${esc(obj.desc)}</div>`:''}
    </div>
    <div style="display:flex;gap:4px;align-items:center">
      <button class="btn bsm bprimary" onclick="mjOpenGiveItem(${i})">ðŸŽ Donner</button>
      <button class="btn bsm" style="color:#e53935;border-color:#e53935" onclick="mjDeleteItem(${i})">âœ•</button>
    </div>
  </div>`;}).join(''):`<div style="color:var(--text3);font-size:12px;font-style:italic;text-align:center;padding:16px">Aucun objet dans votre liste. CrÃ©ez-en un ou cherchez dans le compendium.</div>`;

  const rarFilterBar=['','Commun','Peu commun','Rare','TrÃ¨s rare','LÃ©gendaire','Artefact'].map(r=>{
  ...
  const compSection=ITEMS_DB
    ?`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:12px">
        <div style="font-size:12px;font-weight:600;color:var(--cp);margin-bottom:8px">ðŸ“š Compendium â€” ${ITEMS_DB.length.toLocaleString()} objets D&D 5e</div>
        <div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px">${rarFilterBar}</div>
        <input class="fi" id="itemSearch" placeholder="Chercher : Ã‰pÃ©e longue, Potion de soins, Anneau..." oninput="mjFilterItems(this.value)" onfocus="mjFilterItems(this.value)" style="margin-bottom:6px">
    ...
    :`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:12px;text-align:center">
        ...
        <button class="btn bsm bprimary" onclick="loadItemsDB(()=>renderMJContent())">ðŸ“š Charger le compendium d'objets</button>
      </div>`;

  return`<div>
    ${compSection}
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
      <div class="pt" style="margin-bottom:0;padding-bottom:0;border-bottom:none;font-size:13px">ðŸ’° Inventaire du MJ</div>
      <button class="btn bsm bprimary" onclick="mjOpenNewItem()">+ CrÃ©er manuellement</button>
    </div>
    ${itemList}
  </div>`;
}
...
function _mjGetRarity(it){
  const RARITY_MAP={'trÃ¨s rare':'TrÃ¨s rare','peu commun':'Peu commun','lÃ©gendaire':'LÃ©gendaire','artefact':'Artefact','commun':'Commun','rare':'Rare'};
  ...
  el.innerHTML=items.length?items.map(({i,it})=>{
    ...
    return`<div class="charlib-item" style="padding:6px 8px;cursor:pointer;flex-direction:column;align-items:flex-start;gap:2px" onclick="mjAddItemFromCompendium(${i})">
      <div style="display:flex;align-items:center;gap:6px;width:100%">
        <span style="font-size:16px;flex-shrink:0">${_TYPE_ICON[it.t]||'ðŸ“¦'}</span>
        <div style="flex:1;min-width:0">
          <div style="font-size:12px;font-weight:600">${esc(it.n)}</div>
          <div style="font-size:11px;color:var(--text3)">${esc(it.d||'')}${it.mg?' âœ¨':''}</div>
        </div>
        <span style="color:var(--cp);font-size:11px;white-space:nowrap;flex-shrink:0">+ Ajouter</span>
      </div>
      ${rar?`<div style="display:flex;gap:6px;align-items:center;padding-left:22px">${_mjRarBadge(rar)}${ri?`<span style="font-size:10px;color:var(--text3)">${ri.level} Â· ${ri.price}</span>`:''}</div>`:''}
    </div>`;
  }).join(''):`<div style="font-size:12px;color:var(--text3);text-align:center;padding:8px">Aucun rÃ©sultat${rarF?' pour la raretÃ© "'+esc(rarF)+'"':q.trim()?' pour "'+esc(q)+'"':''}</div>`;
}
...
async function mjAddItemFromCompendium(idx){
  ...
  _mjObjets.push(item);
  await saveMJData();
  showToast(`âœ… "${item.name}" ajoutÃ© Ã  votre inventaire !`);
  renderMJContent();
}

function mjOpenNewItem(){
  openModal(`<div class="pt">ðŸ’° Nouvel objet</div>
    <div class="g2" style="gap:8px;margin-bottom:8px">
      <div><div class="fl mb6">Nom</div><input class="fi" id="item_name" placeholder="Ã‰pÃ©e courte +1..."></div>
    ...
    <div class="g2" style="gap:8px;margin-bottom:8px">
      <div><div class="fl mb6">RaretÃ©</div>
        <select class="fi" id="item_rarity">
          ${['Commun','Peu commun','Rare','TrÃ¨s rare','LÃ©gendaire','Artefact'].map(r=>`<option>${r}</option>`).join('')}
        </select>
      </div>
    ...
    <label style="display:flex;align-items:center;gap:8px;font-size:12px;color:var(--text2);cursor:pointer;margin-bottom:16px">
      ...
      <span>ðŸ”— NÃ©cessite un lien (attunement)</span>
    </label>
    <div style="display:flex;gap:8px">
      <button class="btn" style="flex:1" onclick="closeModal()">Annuler</button>
      <button class="btn bac" style="flex:2" onclick="mjSaveItem()">ðŸ’¾ Sauvegarder</button>
    </div>`);
}
...
async function mjSaveItem(){
  ...
  _mjObjets.push(item);
  await saveMJData();
  closeModal();showToast('âœ… Objet crÃ©Ã© !');renderMJContent();
}

async function mjDeleteItem(i){
  _mjObjets.splice(i,1);
  await saveMJData();showToast('ðŸ—‘ Objet supprimÃ©.');renderMJContent();
}
...
function mjOpenGiveItem(itemIdx){
  ...
  openModal(`<div class="pt">ðŸŽ Donner "${esc(item.name||'?')}"</div>
    ...
    ${_mjPlayersData.map((pp,pi)=>`<div class="charlib-item" onclick="mjGiveItem(${itemIdx},${pi})">
      <span style="font-size:20px">${pp.avatar||'âš”'}</span>
      <div style="flex:1">
      ...
      <span style="color:var(--cp);font-size:11px">Donner â†’</span>
    </div>`).join('')}
...
async function mjGiveItem(itemIdx,playerIdx){
  ...
  try{
    ...
    if(!charDoc.exists){showToast('âŒ Personnage introuvable.');return;}
    ...
    closeModal();
    showToast(`âœ… "${esc(item.name||'?')}" donnÃ© Ã  ${esc(pp.charData&&pp.charData.charName||pp.playerName||'?')} !`);
  }catch(e){showToast('âŒ Erreur : '+e.message);}
}

// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// TAB JOURNAL MJ (dans l'Ã©cran MJ)
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
