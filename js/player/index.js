...
function showToast(html,duration=3000){let t=document.getElementById('toast');if(!t){t=document.createElement('div');t.id='toast';t.style.cssText='position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:var(--surface2);border:1px solid var(--cp);border-radius:8px;padding:10px 18px;font-size:18px;color:var(--text);z-index:9999;max-width:400px;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,.5)';document.body.appendChild(t);}t.innerHTML=html;t.style.display='block';clearTimeout(t._timer);t._timer=setTimeout(()=>{t.style.display='none';},duration);}
...
function showIRLRoll(html){
  ...
  ov.innerHTML=`<div style="background:var(--surface);border:2px solid var(--cp);border-radius:16px;padding:28px 24px;max-width:420px;width:100%;text-align:center;box-shadow:0 8px 40px rgba(0,0,0,.8)">
    <div style="font-size:17px;color:var(--cp);text-transform:uppercase;letter-spacing:.1em;margin-bottom:14px">🪄 Mode dés IRL — Lance tes dés !</div>
    <div style="font-size:24px;color:var(--text);line-height:1.8;margin-bottom:24px">${html}</div>
    <button class="btn bprimary" style="width:100%;font-size:22px;padding:14px;font-weight:700;letter-spacing:.03em" onclick="document.getElementById('irlRollOverlay').style.display='none'">✓ J'ai lancé !</button>
  </div>`;
...
function _openDiceShortcuts(){
  ...
  sp.innerHTML=`
    <div style="display:flex;align-items:center;gap:8px">
      <span style="font-size:18px;color:var(--cp);background:var(--surface);padding:4px 10px;border-radius:20px;border:1px solid var(--border);white-space:nowrap">Journal</span>
...
function _irlToggleHtml(){
  ...
  return`<div style="display:flex;align-items:center;justify-content:space-between;padding:7px 10px;border-radius:8px;background:${irl?'rgba(255,152,0,.1)':'var(--surface2)'};border:1px solid ${irl?'rgba(255,152,0,.4)':'var(--border)'};margin-bottom:12px;cursor:pointer" onclick="toggleIRLMode()">
    <div><div style="font-size:18px;font-weight:600;color:${irl?'#ff9800':'var(--text2)'}">${irl?'🪄 Mode IRL actif':'🎲 Mode virtuel actif'}</div><div style="font-size:15px;color:var(--text3)">${irl?'Vous lancez vos vrais dés':'Les dés sont lancés automatiquement'}</div></div>
...
function renderDicePanel(){
  ...
  const p=P();
  if(!p||!p.abilities){
    panel.innerHTML=`<div style="font-family:var(--F);font-size:18px;color:var(--cp);letter-spacing:.06em;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center"><span>🎲 Lanceur de dés</span><span onclick="toggleDicePanel()" style="cursor:pointer;color:var(--text3);font-size:22px">×</span></div>
    ${_irlToggleHtml()}
    <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:12px">${['d4','d6','d8','d10','d12','d20','d100'].map(d=>`<button onclick="diceRollFree('${d}')" style="padding:5px 9px;border:1px solid var(--border);border-radius:6px;font-size:18px;cursor:pointer;background:var(--surface2);color:var(--text2);font-family:var(--B)" onmouseenter="this.style.borderColor='var(--cp)';this.style.color='var(--cp)'" onmouseleave="this.style.borderColor='var(--border)';this.style.color='var(--text2)'">${d}</button>`).join('')}</div>
    <div id="diceResultFree" style="padding:8px;background:var(--surface2);border-radius:6px;display:none;font-size:19px;font-weight:600;color:var(--cp);text-align:center;margin-bottom:8px"></div>
    <div style="font-size:17px;color:var(--text3);text-align:center;font-style:italic">Entrez dans une campagne pour les jets de caractéristiques.</div>`;
    return;
  }
  ...
  panel.innerHTML=`
  <div style="font-family:var(--F);font-size:18px;color:var(--cp);letter-spacing:.06em;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
    <span>🎲 Lanceur de dés</span>
    <span onclick="toggleDicePanel()" style="cursor:pointer;color:var(--text3);font-size:22px">×</span>
  </div>
  ${_irlToggleHtml()}

  <!-- Statuts actifs -->
  ${(()=>{const p=P();const activeStatus=(p.statuses||[]).filter(s=>s.rollPenalty||s.rollBonus||s.name==='Invisible');if(!activeStatus.length)return'';return`<div style="padding:6px 8px;background:var(--surface2);border-radius:6px;margin-bottom:10px;font-size:17px"><span style="color:var(--text3)">Statuts actifs :</span> ${activeStatus.map(s=>`<span class="status-badge ${s.type}" style="font-size:15px;padding:2px 6px">${s.icon} ${esc(s.name)}</span>`).join(' ')}</div>`;})()}

  <!-- Dés libres -->
  <div style="font-size:17px;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Dés libres</div>
  <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:12px">
    ${['d4','d6','d8','d10','d12','d20','d100'].map(d=>`<button onclick="diceRoll('${d}','${d} libre')" style="padding:5px 9px;border:1px solid var(--border);border-radius:6px;font-size:18px;cursor:pointer;background:var(--surface2);color:var(--text2);transition:all .15s;font-family:var(--B)" onmouseenter="this.style.borderColor='var(--cp)';this.style.color='var(--cp)'" onmouseleave="this.style.borderColor='var(--border)';this.style.color='var(--text2)'">${d}</button>`).join('')}
  </div>
  <div style="display:flex;gap:6px;margin-bottom:14px;align-items:center">
    <input id="diceQty" type="number" min="1" max="20" value="1" style="width:48px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;padding:5px 6px;color:var(--text);font-size:18px;text-align:center;outline:none">
    <span style="color:var(--text3);font-size:18px">×</span>
    <select id="diceType" style="flex:1;background:var(--surface2);border:1px solid var(--border);border-radius:6px;padding:5px 8px;color:var(--text);font-size:18px;outline:none">
      ${['d4','d6','d8','d10','d12','d20','d100'].map(d=>`<option>${d}</option>`).join('')}
    </select>
    <button onclick="diceRollCustom()" style="padding:5px 12px;border:1px solid var(--cp);border-radius:6px;font-size:18px;cursor:pointer;background:var(--cp);color:#1a1400;font-weight:600;font-family:var(--B)">Lancer</button>
  </div>

  <!-- Jets de caractéristiques -->
  <div style="font-size:17px;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Jets de caractéristiques</div>
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;margin-bottom:12px">
    ${ABILITIES.map((ab,i)=>{
      ...
      return`<button onclick="diceRoll('d20','${ab}',${m},'${rollType}')" style="padding:6px 4px;border:1px solid ${btnBorder};border-radius:6px;font-size:17px;cursor:pointer;background:${btnBg};color:${btnColor};transition:all .15s;font-family:var(--B);text-align:center" onmouseenter="this.style.borderColor='var(--cp)';this.style.color='var(--cp)'" onmouseleave="this.style.borderColor='${btnBorder}';this.style.color='${btnColor}'">
        <div style="font-weight:600">${ABILITIES_SH[i]}</div>
        <div style="color:${isRageFor?'#e53935':(_isTotemOursFor?'#4caf50':'var(--cp)')};font-size:18px">${fmt(m)}${isRageFor?' 🔥':(_isTotemOursFor?' 🐻':(_isPuissFor?' 💪':''))}</div>
      </button>`;
    }).join('')}
  </div>

  <!-- Jets de sauvegarde -->
  <div style="font-size:17px;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Jets de sauvegarde</div>
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;margin-bottom:12px">
    ${ABILITIES_SH.map((ab,i)=>{
      ...
      return`<button onclick="diceRoll('d20','JS ${ab}',${m},'${rollType}')" style="padding:6px 4px;border:1px solid ${btnBorder};border-radius:6px;font-size:17px;cursor:pointer;background:${btnBg};color:${btnColor};transition:all .15s;font-family:var(--B);text-align:center" onmouseenter="this.style.borderColor='var(--cp)';this.style.color='var(--cp)'" onmouseleave="this.style.borderColor='${btnBorder}';this.style.color='${btnColor}'">
        <div style="font-weight:600">${ab}</div>
        <div style="font-size:18px">${fmt(m)}${isRageFor?' 🔥':(_isTotemOursSave?' 🐻':(_isPuissSave?' 💪':''))}</div>
      </button>`;
    }).join('')}
  </div>

  <!-- Compétences -->
  <div style="font-size:17px;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Compétences</div>
  <div style="display:flex;flex-direction:column;gap:3px;margin-bottom:12px">
    ${SKILLS.map(sk=>{
      ...
      return`<button onclick="diceRoll('d20','${sk.name}',${bonus},'${rollType}')" style="display:flex;align-items:center;gap:8px;padding:5px 8px;border:1px solid ${btnBorder};border-radius:6px;font-size:18px;cursor:pointer;background:${_skBg};color:var(--text2);transition:all .15s;font-family:var(--B);text-align:left;width:100%" onmouseenter="this.style.background='var(--surface3)'" onmouseleave="this.style.background='${_skBg}'">
        ...
        <span style="font-size:17px;color:var(--text3)">${ABILITIES_SH[sk.ab]}</span>
  ...
  <div style="font-size:17px;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Environnement</div>
  <div style="margin-bottom:12px">
    <div style="padding:7px 9px;background:var(--surface2);border-radius:6px;margin-bottom:6px;font-size:17px">
      <div style="font-weight:600;color:var(--text2);margin-bottom:3px">🦘 Sauter <span style="font-size:15px;font-weight:400;color:var(--text3)">(automatique — pas de jet)</span></div>
    ...
    <div style="display:flex;flex-direction:column;gap:3px">
      ...
      ].map(a=>{const h=a.rt==='for-carac';return`<button onclick="diceRoll('d20','${a.l}',${a.b},'${a.rt}')" style="display:flex;align-items:center;gap:8px;padding:5px 8px;border:1px solid ${h?_envBtnBorder:'var(--border)'};border-radius:6px;font-size:18px;cursor:pointer;background:${h?_envBtnBg:'var(--surface2)'};color:var(--text2);transition:all .15s;font-family:var(--B);text-align:left;width:100%" onmouseenter="this.style.background='var(--surface3)'" onmouseleave="this.style.background='${h?_envBtnBg:'var(--surface2)'}'"><span style="flex:1;color:${h?_envBtnColor:'var(--text2)'}">${a.l}${h?_envBtnIcon:''}</span><span style="font-size:15px;color:var(--text3);margin-right:4px">${a.dd}</span><span style="font-weight:600;color:var(--cp);min-width:28px;text-align:right">${fmt(a.b)}</span></button>`;}).join('')}
    </div>
  </div>

  <!-- Historique -->
  ${diceHistory.length?`<div style="font-size:17px;color:var(--text3);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Derniers lancers</div>
  <div id="diceHistoryList" style="display:flex;flex-direction:column;gap:3px">
    ${diceHistory.slice().reverse().map(h=>`<div style="display:flex;justify-content:space-between;padding:4px 8px;background:var(--surface2);border-radius:4px;font-size:18px">
  ...
  if(typeof isMJ==='function'&&isMJ()){
    ...
    openWideModal(`<div class="pt">🤫 Chuchoter à un joueur</div>
      <div style="margin-bottom:10px">${players.length?players.map((pl,i)=>`<div class="lu-choice${_whisperTarget===i?' selected':''}" style="padding:8px 12px;margin-bottom:6px;cursor:pointer" onclick="_whisperTarget=${i};document.querySelectorAll('#modal .lu-choice').forEach((el,j)=>{el.classList.toggle('selected',j===${i});})">
        <div style="font-size:18px;font-weight:600">${esc(pl.playerName||'Joueur')}</div>
        <div style="font-size:17px;color:var(--text3)">${esc((pl.charData||{}).charName||'?')}</div>
      </div>`).join(''):'<div style="font-size:18px;color:var(--text3);padding:8px">Aucun joueur connecté.</div>'}</div>
      <textarea id="whisperMsg" placeholder="Message secret..." style="width:100%;box-sizing:border-box;min-height:72px;padding:8px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:18px;resize:vertical;margin-bottom:8px"></textarea>
      <div style="display:flex;gap:8px">
...
async function _openPlayerWhisperModal(){
  ...
  openModal(`<div style="display:flex;flex-direction:column;gap:8px">
    ...
    <div id="whisperRecipList" style="display:flex;gap:6px;flex-wrap:wrap;padding:2px 0"><div style="font-size:18px;color:var(--text3)">Chargement…</div></div>
    <div id="whisperHistory" style="min-height:80px;max-height:200px;overflow-y:auto;background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:8px">
      <div style="font-size:18px;color:var(--text3);font-style:italic;text-align:center;padding:8px">Sélectionnez un destinataire.</div>
    </div>
  ...
  }catch(e){const el=document.getElementById('whisperRecipList');if(el)el.innerHTML='<div style="font-size:18px;color:#e53935">Erreur de chargement.</div>';}
}
...
function _whisperConvHtml(recipUid){
  ...
  if(!msgs.length)return'<div style="font-size:18px;color:var(--text3);font-style:italic;text-align:center;padding:8px">Aucun message échangé.</div>';
  return msgs.map(w=>{
    const isSent=!!w.sent;
    ...
    return`<div style="margin-bottom:6px;display:flex;flex-direction:column;align-items:${isSent?'flex-end':'flex-start'}">
      <div style="max-width:82%;background:${isSent?'rgba(200,168,75,.12)':'var(--surface)'};border:1px solid ${isSent?'rgba(200,168,75,.35)':'var(--border)'};border-radius:${isSent?'10px 10px 2px 10px':'10px 10px 10px 2px'};padding:5px 10px">
        <div style="font-size:18px;color:var(--text);line-height:1.4">${esc(w.message||'')}</div>
      </div>
      ${ts?`<div style="font-size:15px;color:var(--text3);margin-top:2px">${ts}</div>`:''}
    </div>`;
  }).join('');
}
function _refreshWhisperHistoryDisplay(){
  ...
  if(idx<0||!recips[idx]){
    el.innerHTML='<div style="font-size:18px;color:var(--text3);font-style:italic;text-align:center;padding:8px">Sélectionnez un destinataire.</div>';
    return;
  }
...
function _luckyCheckRolls(rolls,idx,onDone){
  ...
  openModal('<div style="text-align:center;padding:20px 16px"><div style="font-size:36px;margin-bottom:8px">🍀</div><div class="pt" style="margin-bottom:6px">Chanceux</div><div style="font-size:19px;color:var(--text2);margin-bottom:20px">Vous avez obtenu un <strong style="color:#e53935;font-size:25px">1</strong> !<br>Voulez-vous relancer ce dé ?<br><span style="font-size:17px;color:var(--text3)">∞ Illimité — fonctionne même avec avantage/désavantage</span></div><div style="display:flex;gap:8px;justify-content:center"><button class="btn bprimary" style="min-width:80px" onclick="_luckyReroll()">✅ Oui</button><button class="btn" style="min-width:80px" onclick="_luckySkip()">❌ Non</button></div></div>');
}
...
function diceRoll(die,label,bonus=0,rollType=''){
  ...
  if(_isIRLMode()){
    const advNote=effects.hasAdv&&!effects.hasDisadv?'<div style="margin-top:8px;color:#4caf50;font-size:19px">🟢 AVANTAGE — 2d20, garde le plus haut</div>':(effects.hasDisadv&&!effects.hasAdv?'<div style="margin-top:8px;color:#e53935;font-size:19px">🔴 DÉSAVANTAGE — 2d20, garde le plus bas</div>':'');
    const bonusDieNote=effects.bonusDie?` + <span style="color:#ffd54f">${effects.bonusDie}</span>`:'';
    const luckyNote=_isHalfling(p)&&die==='d20'?'<div style="margin-top:6px;font-size:18px;color:#8d6e63">🍀 Si résultat = 1, vous pouvez relancer</div>':'';
  ...
  function _finishRoll(finalRolls){
    ...
    if(barbLvl>=18&&(rollType==='for-carac'||rollType==='for-save')&&total<p.abilities[0]){
      puissanceTag=` <span style="font-size:15px;color:#ff9800">💪 →${p.abilities[0]} (Puissance indomptable)</span>`;
      total=p.abilities[0];
    }
    ...
    if(fr2!==null){
      ...
      msg+=effects.hasAdv?` <span style="font-size:15px;color:#4caf50">AVANTAGE</span>`:`<span style="font-size:15px;color:#e53935"> DÉSAVANTAGE</span>`;
...
function diceRollFree(d){
  ...
  if(el){el.style.display='block';el.innerHTML=`🎲 ${d} : <strong style="font-size:25px">${r}</strong>${r===n?' 🎉':r===1&&n>4?' 💀':''}`;}
}
function diceRollCustom(){
  ...
  if(diceHistory.length>10)diceHistory.shift();
  showToast(`<strong>${label}</strong> : [${rolls.join(', ')}] = <strong style="font-size:22px;color:var(--cp)">${total}</strong>`);
  if(diceOpen)renderDicePanel();
}
