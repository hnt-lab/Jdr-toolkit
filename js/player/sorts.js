...
function renderConcPanel(p){
  ...
  return `<div class="conc-panel-pulse" style="background:var(--surface2);border:2px solid var(--cp);border-radius:2px;padding:12px;margin-bottom:10px">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
      <span style="font-size:18px">🎯</span>
      <div style="flex:1"><div style="font-size:13px;font-weight:700;color:var(--cp)">Concentration</div>${concSpell?`<div style="font-size:13px;color:var(--text2);margin-top:1px">${esc(concSpell)}</div>`:''}</div>
      <button class="btn bsm" style="font-size:13px" onclick="toggleConcentration()">✕ Briser</button>
    </div>
    <div style="font-size:13px;color:var(--text3);margin-bottom:10px;line-height:1.5">Si tu subis des dégâts, lance un <strong style="color:var(--text2)">JS CON</strong> (DD = max 10 ou moitié des dégâts reçus). Échec = concentration brisée.</div>
    <div style="display:flex;gap:6px;align-items:center">
      <input type="number" id="concDmgInput" placeholder="Dégâts reçus" min="0" style="flex:1;padding:7px 10px;background:var(--surface);border:1px solid var(--border);border-radius:2px;color:var(--text);font-size:13px">
...
function tabSorts(p){
  ...
  if(_sortSubTab === 'compendium' && (userIsMJ||prepCaster)){
    return`<div>${subBar}${prepCaster&&!userIsMJ?`<div style="font-size:13px;color:var(--text3);padding:4px 0 8px">Cliquez sur un sort pour l'ajouter à votre liste. Vous pouvez ensuite le préparer depuis "Mes sorts".</div>`:''}${renderCompendiumSearch(p)}</div>`;
  }
  ...
  if(prepCaster){
    ...
    prepInfo = `<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;padding:8px;background:var(--surface2);border-radius:2px;font-size:13px">
      <span style="color:var(--text2)">Sorts préparés :</span>
      <strong style="color:${prepCount>prepMax?'var(--danger)':'var(--good)'};font-size:14px">${prepCount}/${prepMax}</strong>
      <span style="color:var(--text3);font-size:13px">(Cliquez sur un sort pour le préparer / retirer)</span>
    </div>`;
  }
  ...
  const slotHtml=(()=>{
    if(warlockSlots){
      return`<div style="margin-bottom:10px;padding:10px;background:var(--surface2);border-radius:2px">
        <div style="font-size:13px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Magie de pacte — Niv.${warlockSlots[1]}</div>
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <span style="font-size:13px;color:var(--text3)">${warlockSlots[0]-(slotsUsed[9]||0)}/${warlockSlots[0]}</span>
    ...
    const rows=slots.map((total,ni)=>total?`<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
      <span style="font-size:13px;color:var(--text2);width:46px">Niv. ${ni+1}</span>
      ...
      <span style="font-size:12px;color:var(--text3)">${total-(slotsUsed[ni]||0)}/${total}</span>
    </div>`:''). join('');
    return`<div style="margin-bottom:10px;padding:10px;background:var(--surface2);border-radius:2px">
      <div style="font-size:13px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Emplacements de sorts</div>
      ${rows}
  ...
  const circleSection=(()=>{
    ...
    if(!terrain)return`<div style="font-size:13px;color:var(--text3);padding:8px 0;font-style:italic">⭐ Sorts du Cercle — terrain non configuré. Réinitialisez votre personnage (respec MJ) pour choisir un terrain.</div>`;
...
function _renderLongRestPrepModal(p){
  ...
  const row=s=>{const on=_lrPrep.sel.has(s.name);const dis=!on&&full;return`<div class="sk-choice${on?' selected':dis?' disabled':''}" onclick="${dis?'':`_lrPrepToggle('${jsq(s.name)}')`}"><span class="sk-dot${on?' p':''}"></span><span style="flex:1;font-size:13px">${esc(s.name)}</span><span style="font-size:13px;color:var(--text3)">${esc(s.school||'')}</span>${on?'<span style="color:var(--cp)">✓</span>':''}</div>`;};
  let body='';
  ...
  const alwaysHtml=always.size?`<div style="font-size:13px;color:var(--text3);margin-bottom:8px">⭐ Toujours préparés (hors quota) : ${[...always].map(esc).join(', ')}</div>`:'';
  openModal(`<div class="pt">🌙 Préparation des sorts — ${esc(className)}</div>
    ${_lrPrep.multi?`<div style="font-size:13px;color:var(--warn);margin-bottom:8px">Multiclasse : préparation de la classe principale (${esc(className)}). Les autres classes se préparent depuis l'onglet Sorts.</div>`:''}
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;font-size:13px"><span style="color:var(--text2)">Sorts préparés :</span><strong style="color:${_lrPrep.sel.size>N?'var(--danger)':'var(--good)'};font-size:15px">${_lrPrep.sel.size}/${N}</strong></div>
    ${alwaysHtml}
    <input type="text" placeholder="🔍 Rechercher..." value="${esc(_lrPrep.search||'')}" oninput="_lrPrepSearch(this.value)" style="width:100%;box-sizing:border-box;padding:7px 10px;background:var(--surface2);border:1px solid var(--border);border-radius:2px;color:var(--text);font-size:13px;margin-bottom:8px">
    ...
    ${!isLoaded?`<div style="text-align:center;padding:16px"><button class="btn bprimary" onclick="loadSpellsDB(()=>render())">⬇️ Charger le compendium</button></div>`:`
      <div style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap">
        <input type="text" placeholder="🔍 Rechercher..." value="${esc(_learnSpellSearch.q)}"
          ...
          style="flex:2;min-width:140px;padding:7px 10px;background:var(--surface2);border:1px solid var(--border);border-radius:2px;color:var(--text);font-size:13px">
        <select onchange="_learnSpellSearch.school=this.value;render()"
          style="flex:1;min-width:100px;padding:7px;background:var(--surface2);border:1px solid var(--border);border-radius:2px;color:var(--text);font-size:13px">
          <option value="">Toutes écoles</option>
          ${schools.map(sc=>`<option value="${sc}"${_learnSpellSearch.school===sc?' selected':''}>${sc}</option>`).join('')}
        </select>
      </div>
      <div style="font-size:13px;color:var(--text3);margin-bottom:8px">${results.length} sort(s) disponible(s) à copier</div>
        ...
        ${results.map(s=>{
          ...
          return`<div class="g-sub" style="padding:8px 10px;margin-bottom:5px;display:flex;align-items:center;gap:8px">
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:13px;color:${canAfford?'var(--good)':'var(--danger)'};margin-top:2px">${cost} po — ${canAfford?'✓ Fonds suffisants':'✗ Fonds insuffisants'}</div>
            </div>
    ...
    `:`
      <div style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap">
        ...
        <select onchange="_spellSearch.lvl=this.value;render()"
          style="padding:7px;background:var(--surface2);border:1px solid var(--border);border-radius:2px;color:var(--text);font-size:13px">
      ...
      <div style="font-size:13px;color:var(--text3);margin-bottom:8px">${results.length} résultat(s) ${results.length===50?'(50 max — affinez la recherche)':''}</div>
        ...
        ${results.map(s=>{
          ...
          return`<div class="g-sub" style="padding:8px 10px;margin-bottom:5px;display:flex;align-items:center;gap:8px">
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:13px;color:var(--text3);margin-top:2px">${s.classes.join(', ')}</div>
            </div>
            ${already
      ...
      <div style="margin-top:8px;text-align:right">
        <button class="btn bsm" onclick="clearSpellsCache();render()" style="font-size:12px;color:var(--text3)">🗑 Vider le cache</button>
      </div>
    `}
  </div>`;
}
...
function _openSupportSpellModal(name,def,spellMod,upcastLvl){
  ...
  const rowStyle='display:flex;align-items:center;gap:10px;padding:8px;background:var(--surface2);border:1px solid var(--border);border-radius:2px;margin-bottom:6px';
  if(def.maxTargets===1){
    const listHtml=allTargets.map((t,i)=>`<div style="${rowStyle};cursor:pointer" onclick="closeModal();_applySupport([${i}])">`+
      ...
      `<span style="font-size:13px;color:var(--cp)">${def.heal?healLabel:'+1'+def.die} →</span></div>`).join('');
    openModal(`<div class="pt">${def.icon} ${esc(name)}</div><div style="font-size:13px;color:var(--text3);margin-bottom:12px">${def.heal?`Soigne : <strong style="color:var(--cp)">${healLabel}</strong>`:esc(def.detail)}</div>${listHtml}<button class="btn" style="width:100%;margin-top:4px" onclick="closeModal()">Annuler</button>`);
  }else{
    const listHtml=allTargets.map((t,i)=>`<div id="starg_${i}" style="${rowStyle};cursor:pointer" onclick="_toggleSupTarget(${i})">`+
      `<div id="stchk_${i}" style="width:18px;height:18px;border-radius:2px;border:2px solid var(--border);background:var(--surface);flex-shrink:0"></div>`+
  ...
  if(isConc&&(p.statuses||[]).some(s=>s.name==='Concentration')){
    if(!_forceConc){
      openModal(`<div class="pt">🔵 Concentration déjà active</div>
        <p style="font-size:13px;color:var(--text2);line-height:1.5;margin-bottom:6px">Tu te concentres déjà sur <strong style="color:var(--cp)">${esc(p.concentrationSpell||'un sort')}</strong>.</p>
        <p style="font-size:13px;color:var(--text2);line-height:1.5;margin-bottom:14px">On ne maintient qu'<strong>une seule concentration</strong> à la fois : lancer <strong style="color:var(--cp)">${esc(name)}</strong> mettra fin à l'effet précédent (et retirera ses bonus aux alliés concernés).</p>
...
function _finalizeCast(name,d,upcastLvl,isConc){
  ...
  if(_isIRLMode()){
    ...
    if(upcastLvl)html+=` <span style="font-size:13px;color:var(--cp)">(amplifié niv.${upcastLvl})</span>`;
    if(isConc)html+=`<br><span style="font-size:13px;color:var(--cp)">🎯 Concentration activée</span>`;
    if(save)html+=`<br>JS <strong>${esc(save)}</strong> — DD <strong style="font-size:15px;color:var(--cp)">${spellDC}</strong>`;
