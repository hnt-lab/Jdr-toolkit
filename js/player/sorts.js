...
function tabSorts(p){
  ...
  const userIsMJ = isMJ(); // via mjMode â€” sera connectÃ© aux rÃ´les Firebase
  ...
  const subBar = hasMultiSubs?`<div style="display:flex;gap:6px;margin-bottom:12px">
    <button class="btn${_sortSubTab==='mes-sorts'?' bprimary':''}" onclick="_sortSubTab='mes-sorts';render()" style="flex:1">ðŸ“– Mes sorts</button>
    ${userIsMJ?`<button class="btn${_sortSubTab==='compendium'?' bprimary':''}" onclick="_sortSubTab='compendium';render()" style="flex:1">ðŸ“š Ajouter (MJ)</button>`:''}
    ${magLvl?`<button class="btn${_sortSubTab==='apprendre'?' bprimary':''}" onclick="_sortSubTab='apprendre';render()" style="flex:1">ðŸ“œ Apprendre</button>`:''}
  </div>`:'';
  ...
  const conSaveLabel=`JS CON ${fmt(conSaveBonus)}${_concHasCON?' (maÃ®trise incluse)':''}`;
  ...
  const concBtn=isConcentrating?`<div style="background:var(--surface2);border:2px solid #ffd54f;border-radius:10px;padding:12px;margin-bottom:10px">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
      ...
      <button class="btn bsm" style="font-size:11px" onclick="toggleConcentration()">âœ• Briser</button>
    </div>
    <div style="font-size:11px;color:var(--text3);margin-bottom:10px;line-height:1.5">Si tu subis des dÃ©gÃ¢ts, lance un <strong style="color:var(--text2)">JS CON</strong> (DD = max 10 ou moitiÃ© des dÃ©gÃ¢ts reÃ§us). Ã‰chec = concentration brisÃ©e automatiquement.</div>
    <div style="display:flex;gap:6px;align-items:center">
      <input type="number" id="concDmgInput" placeholder="DÃ©gÃ¢ts reÃ§us" min="0" style="flex:1;padding:7px 10px;background:var(--surface);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:13px">
      <button class="btn bac" style="white-space:nowrap;font-size:13px" onclick="rollConcSave(parseInt(document.getElementById('concDmgInput')?.value)||0)">ðŸŽ² ${conSaveLabel}</button>
    </div>
  </div>`:`<div style="display:flex;gap:6px;margin-bottom:10px"><input id="concSpellInput" placeholder="Sort de concentration (optionnel)â€¦" style="flex:1;padding:7px 10px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:13px"><button class="btn" style="white-space:nowrap;font-size:13px" onclick="activateConcentration()">ðŸŽ¯ Concentrer</button></div>`;
  ...
  if(prepCaster){
    ...
    prepInfo = `<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;padding:8px;background:var(--surface2);border-radius:8px;font-size:12px">
      <span style="color:var(--text2)">Sorts prÃ©parÃ©s :</span>
      <strong style="color:${prepCount>prepMax?'#e53935':'#4caf50'};font-size:14px">${prepCount}/${prepMax}</strong>
      <span style="color:var(--text3);font-size:11px">(Cliquez sur un sort pour le prÃ©parer / retirer)</span>
    </div>`;
  }
  ...
  const slotHtml=(()=>{
    if(warlockSlots){
      return`<div style="margin-bottom:10px;padding:10px;background:var(--surface2);border-radius:8px">
        <div style="font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Magie de pacte â€” Niv.${warlockSlots[1]}</div>
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <button class="btn bsm" onclick="P().spellSlotsUsed=P().spellSlotsUsed||[];P().spellSlotsUsed[9]=0;render()">â†º</button>
        </div>
      </div>`;
    }
    if(!slots)return'';
    ...
    return`<div style="margin-bottom:10px;padding:10px;background:var(--surface2);border-radius:8px">
      ...
      ${rows}
      <button class="btn bsm" style="margin-top:4px" onclick="upd('spellSlotsUsed',[]);render()">â†º RÃ©cupÃ©rer tous (repos long)</button>
    </div>`;
  })();
  return`<div>
    ${subBar}
...
function renderLearnSpell(p, magLvl){
  ...
  return`<div>
    <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:12px">
      <div style="font-size:13px;font-weight:600;color:var(--cp);margin-bottom:4px">ðŸ“œ Copier un sort dans votre grimoire</div>
      <div style="font-size:12px;color:var(--text3)">CoÃ»t : 2h + <strong>50 po par niveau du sort</strong> â€” Tous les sorts Magicien disponibles</div>
      <div style="font-size:12px;color:var(--text2);margin-top:4px">Votre trÃ©sor actuel : <strong style="color:var(--cp)">${cur.po||0} po</strong></div>
    </div>
    ${!isLoaded?`<div style="text-align:center;padding:16px"><button class="btn bprimary" onclick="loadSpellsDB(()=>render())">â¬‡ï¸ Charger le compendium</button></div>`:`
      <div style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap">
        <input type="text" placeholder="ðŸ” Rechercher..." value="${esc(_learnSpellSearch.q)}"
        ...
        <select onchange="_learnSpellSearch.school=this.value;render()"
          ...
          <option value="">Toutes Ã©coles</option>
          ${schools.map(sc=>`<option value="${sc}"${_learnSpellSearch.school===sc?' selected':''}>${sc}</option>`).join('')}
        </select>
      </div>
      <div style="font-size:11px;color:var(--text3);margin-bottom:8px">${results.length} sort(s) disponible(s) Ã  copier</div>
        ...
        ${results.map(s=>{
          ...
          return`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:8px 10px;margin-bottom:5px;display:flex;align-items:center;gap:8px">
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--text3)">Niv.${s.level} â€¢ ${esc(s.school)} â€¢ ${esc(s.castTime)}</div>
              <div style="font-size:11px;color:${canAfford?'#4caf50':'#e53935'};margin-top:2px">${cost} po â€” ${canAfford?'âœ“ Fonds suffisants':'âœ— Fonds insuffisants'}</div>
            </div>
...
function learnSpell(name, level, cost){
  ...
  if((cur.po||0) < cost){
    showToast(`âŒ Fonds insuffisants ! Il vous faut ${cost} po (vous avez ${cur.po||0} po)`);
    return;
  }
  if(!p.spells)p.spells=[];
  if(p.spells.find(s=>s.name===name)){showToast('Ce sort est dÃ©jÃ  dans votre grimoire.');return;}
  ...
  saveAll();render();
  showToast(`âœ… "${name}" copiÃ© dans votre grimoire ! âˆ’${cost} po (2h de travail)`);
}
  ...
  return`<div>
    ${!isLoaded?`
      <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:16px;text-align:center;margin-bottom:12px">
        <div style="font-size:14px;font-weight:600;color:var(--cp);margin-bottom:6px">ðŸ“š Compendium de sorts</div>
        <div style="font-size:12px;color:var(--text3);margin-bottom:12px">1213 sorts disponibles. Placez <strong>spells_db.json</strong> dans le mÃªme dossier que la fiche.</div>
        <button class="btn bprimary" onclick="loadSpellsDB(()=>render())">â¬‡ï¸ Charger le compendium</button>
      </div>
    `:`
      <div style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap">
        <input type="text" placeholder="ðŸ” Rechercher un sort..." value="${esc(_spellSearch.q)}"
          oninput="_spellSearch.q=this.value;render()"
          style="flex:2;min-width:140px;padding:7px 10px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:13px">
        <select onchange="_spellSearch.cls=this.value;render()"
          style="flex:1;min-width:100px;padding:7px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:12px">
          <option value="">Toutes classes</option>
          ${['Barbare','Barde','Clerc','Druide','Guerrier','Moine','Paladin','RÃ´deur','Roublard','Ensorceleur','Occultiste','Magicien','Artificier'].map(c=>`<option value="${c}"${_spellSearch.cls===c?' selected':''}>${c}</option>`).join('')}
        </select>
      ...
      <div style="font-size:11px;color:var(--text3);margin-bottom:8px">${results.length} rÃ©sultat(s) ${results.length===50?'(50 max â€” affinez la recherche)':''}</div>
        ...
        ${results.map(s=>{
          ...
          return`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:8px 10px;margin-bottom:5px;display:flex;align-items:center;gap:8px">
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--text3)">${s.level===0?'Cantrip':'Niv.'+s.level} â€¢ ${esc(s.school)} â€¢ ${esc(s.castTime)} â€¢ ${esc(s.range)}</div>
      ...
      <div style="margin-top:8px;text-align:right">
        <button class="btn bsm" onclick="clearSpellsCache();render()" style="font-size:10px;color:var(--text3)">ðŸ—‘ Vider le cache</button>
      </div>
    `}
  </div>`;
}
