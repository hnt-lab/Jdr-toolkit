...
function renderBarde(p) {
  ...
  panels.push(`<div style="margin-bottom:10px;padding:8px;background:var(--surface2);border-radius:6px">
    <div style="font-size:13px;font-weight:600;color:var(--cp);margin-bottom:6px">🎵 Inspiration bardique <span style="font-size:11px;color:var(--cp)">🔸</span></div>
    <div style="font-size:12px;color:var(--text3);margin-bottom:4px">Utilisations (mod CHA = ${Math.max(1,chaM)}) • Récup. repos ${bardeLvl>=5?'court':'long'}</div>
    ...
    <div style="font-size:11px;color:var(--text3);margin-top:6px">Action bonus : donne un dé ${bardDie} à un allié. Il peut l'ajouter à un jet d'attaque, compétence ou sauvegarde.</div>
  </div>`);
  ...
  if (bardeLvl>=2) {
    ...
    panels.push(`<div style="margin-bottom:10px;padding:8px;background:${hasResult?'rgba(76,175,80,.07)':'var(--surface2)'};border-radius:6px;border:1px solid ${hasResult?'rgba(76,175,80,.35)':'var(--border)'}">
      ...
      <div style="font-size:12px;color:var(--text3);margin-bottom:8px">Au prochain repos court, chaque allié qui dépense des dés de vie récupère <strong>${chantDie}</strong> PV supplémentaires. Lance le dé et annonce le résultat.</div>
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        ...
        ${hasResult?`<span style="font-size:16px;font-weight:700;color:#4caf50">+${chantResult} PV</span><span style="font-size:11px;color:var(--text3)">annoncé aux alliés</span>`:''}
      </div>
      ${hasResult?`<div style="font-size:11px;color:var(--text3);margin-top:6px">💡 Alliés : ajoutez <strong style="color:#4caf50">+${chantResult}</strong> à votre récupération de dés de vie. Se réinitialise après le repos.</div>`:''}
    </div>`);
  }
  ...
  if (bardeLvl>=2) {
    panels.push(`<div style="margin-bottom:10px;padding:8px;background:var(--surface2);border-radius:6px">
      ...
      <div style="font-size:12px;color:var(--text3)">+${Math.floor(pb(lvl)/2)} (½ maîtrise) à tous les jets de caractéristique pour lesquels tu n'appliques pas déjà ta maîtrise complète.</div>
    </div>`);
  }
  ...
  if (bardeLvl>=6) {
    ...
    panels.push(`<div style="margin-bottom:10px;padding:8px;background:${cmActive?'rgba(33,150,243,.1)':'var(--surface2)'};border-radius:6px;border:1px solid ${cmActive?'rgba(33,150,243,.5)':'var(--border)'}">
      ...
      <div style="font-size:12px;color:var(--text3)">${cmActive?'🎤 Actif — toi et les alliés à 9m avez l\'avantage aux JS contre charme et peur.':'Action : commence une représentation jusqu\'à la fin de ton prochain tour. Toi + alliés à 9m : avantage aux JS contre charme et peur.'}</div>
    </div>`);
  }
  ...
  if (bardeLvl>=20) {
    panels.push(`<div style="margin-bottom:10px;padding:8px;background:rgba(200,168,75,.08);border-radius:6px;border:1px solid rgba(200,168,75,.4)">
      ...
      <div style="font-size:12px;color:var(--text3)">Quand tu lances l'initiative sans inspiration bardique, tu en regagnes 1 automatiquement.</div>
    </div>`);
  }
  ...
  if (isSavoir && bardeLvl>=3) {
    let savoirContent = `<div style="padding:7px 10px;background:var(--surface2);border-radius:6px;margin-bottom:6px">
      <div style="font-size:13px;font-weight:600;margin-bottom:2px">🗡 Mots cinglants <span style="font-size:11px;color:var(--cp);margin-left:4px">Réaction • 1 inspiration</span></div>
      <div style="font-size:12px;color:var(--text3)">Quand une créature visible fait un jet d'attaque, compétence ou dégâts : dépense 1 inspiration, la cible soustrait le résultat du dé. Décider après le jet, avant l'annonce du MJ. Immunisé si la cible est insensible aux charmes.</div>
    ...
    if (bardeLvl>=6) {
      savoirContent += `<div style="padding:7px 10px;background:var(--surface2);border-radius:6px;margin-bottom:6px">
        <div style="font-size:13px;font-weight:600;margin-bottom:2px">📖 Secrets magiques supplémentaires <span style="font-size:11px;color:var(--text3)">niv.6</span></div>
        <div style="font-size:12px;color:var(--text3)">2 sorts de n'importe quelle classe (en plus des Secrets magiques niv.10/14/18). Ne comptent pas dans le quota de sorts connus.</div>
      </div>`;
    }
    if (bardeLvl>=14) {
      savoirContent += `<div style="padding:7px 10px;background:var(--surface2);border-radius:6px">
        <div style="font-size:13px;font-weight:600;margin-bottom:2px">🎯 Compétence hors-pair <span style="font-size:11px;color:var(--text3)">niv.14</span></div>
        <div style="font-size:12px;color:var(--text3)">Quand tu rates un jet de caractéristique, dépense 1 inspiration bardique et ajoute le résultat du dé. Peut transformer un échec en réussite.</div>
    ...
    panels.push(`<div style="margin-bottom:10px;padding:10px;background:rgba(156,39,176,.07);border:1px solid rgba(156,39,176,.2);border-radius:8px">
      <div style="font-size:13px;font-weight:700;color:#9c27b0;margin-bottom:8px">📚 Collège du savoir</div>
      ${savoirContent}
    </div>`);
  }
  ...
  if (isVaillance && bardeLvl>=3) {
    let vaillanceContent = `<div style="padding:7px 10px;background:var(--surface2);border-radius:6px;margin-bottom:6px">
      <div style="font-size:13px;font-weight:600;margin-bottom:2px">⚔ Inspiration martiale <span style="font-size:11px;color:var(--cp);margin-left:4px">1 inspiration</span></div>
      <div style="font-size:12px;color:var(--text3)">Un allié portant une inspiration bardique peut l'utiliser pour : ajouter le résultat à un jet de dégâts d'arme, OU (réaction) ajouter le résultat à sa CA contre une attaque (après avoir vu le jet).</div>
    </div>`;
    if (bardeLvl>=6) {
      vaillanceContent += `<div style="padding:7px 10px;background:var(--surface2);border-radius:6px;margin-bottom:6px">
        <div style="font-size:13px;font-weight:600;margin-bottom:2px">⚔ Attaque supplémentaire <span style="font-size:11px;color:var(--text3)">niv.6</span></div>
        <div style="font-size:12px;color:var(--text3)">Tu peux attaquer deux fois au lieu d'une lors de l'action Attaquer.</div>
      </div>`;
    }
    if (bardeLvl>=14) {
      vaillanceContent += `<div style="padding:7px 10px;background:var(--surface2);border-radius:6px">
        <div style="font-size:13px;font-weight:600;margin-bottom:2px">✨ Magie de combat <span style="font-size:11px;color:var(--text3)">niv.14</span></div>
        <div style="font-size:12px;color:var(--text3)">Quand tu utilises ton action pour lancer un sort de barde, tu peux faire une attaque avec une arme en action bonus.</div>
      </div>`;
    }
    panels.push(`<div style="margin-bottom:10px;padding:10px;background:rgba(33,150,243,.07);border:1px solid rgba(33,150,243,.2);border-radius:8px">
      <div style="font-size:13px;font-weight:700;color:#2196f3;margin-bottom:8px">🛡 Collège de la vaillance</div>
      ${vaillanceContent}
    </div>`);
  }
...
function openGiveInspirationModal(die, maxCharges) {
  ...
  const listHtml = allies.map((gp,i) => {
    ...
    return `<div style="display:flex;align-items:center;gap:10px;padding:8px;background:${already?'rgba(200,168,75,.06)':'var(--surface2)'};border:1px solid ${already?'var(--cp)':'var(--border)'};border-radius:8px;margin-bottom:6px;cursor:${already?'default':'pointer'}" ${already?'':`onclick="_confirmGiveInspiration('${gp.docId}','${esc(gp.playerName||'')}','${die}',${maxCharges})"`}>
      ...
      <div style="flex:1"><div style="font-size:13px;font-weight:600">${esc(cd.charName||gp.playerName||'Joueur')}</div><div style="font-size:11px;color:var(--text3)">${esc(gp.playerName||'')}</div></div>
      ${already?`<span style="font-size:11px;color:var(--cp);border:1px solid var(--cp);border-radius:8px;padding:2px 8px">🎵 Déjà inspiré</span>`:`<span style="font-size:12px;color:var(--cp)">+1${die} →</span>`}
    </div>`;
  }).join('');
  openModal(`<div class="pt">🎵 Donner l'inspiration bardique</div>
    <div style="font-size:12px;color:var(--text3);margin-bottom:12px">Choisissez l'allié qui reçoit le dé ${die}. Dépense 1 charge.</div>
    ${listHtml}
