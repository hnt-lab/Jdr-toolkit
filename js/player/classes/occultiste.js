...
function renderOccultiste(p) {
  ...
  return cs('cs-warlock',`<div class="panel mb10"><div class="pt" style="display:flex;align-items:center;justify-content:space-between"><div style="display:flex;align-items:center;gap:6px"><span class="mj-drag-handle" title="Déplacer">⠿</span><span>Magie de pacte — Occultiste</span></div><span style="font-size:10px;color:var(--cp);border:1px solid var(--cp);border-radius:10px;padding:2px 8px">Repos court</span></div>
      ...
      ${(()=>{if(!occLvl)return'';const patronPath=(p.features||[]).find(f=>["L'Archifée","Le Fiélon","Le Grand Ancien","Le Génie"].includes(f.name));const isArchifee=patronPath?.name==="L'Archifée";const isFielon=patronPath?.name==="Le Fiélon";const isGrandAncien=patronPath?.name==="Le Grand Ancien";const isGenie=patronPath?.name==="Le Génie";const cc=p.combatCharges||{};let html='';
        ...
        if(isGenie){const kinds=[{id:'Dao',icon:'⛰',el:'Contondant'},{id:'Djinn',icon:'🌪',el:'Tonnerre'},{id:'Efreet',icon:'🔥',el:'Feu'},{id:'Marid',icon:'🌊',el:'Froid'}];const gk=cc['GenieKind']||'';const elem=(kinds.find(k=>k.id===gk)||{}).el||'';const slU=!!cc['SouhaitLimiteUsed'];html+=`<div style="border-top:1px solid var(--border);padding-top:10px;margin-top:8px"><div style="font-size:12px;font-weight:600;color:var(--cp);margin-bottom:6px">🌟 Le Génie</div><div style="font-size:11px;color:var(--text3);margin-bottom:4px">Type de génie (élément) :</div><div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:6px">${kinds.map(k=>`<button class="btn bsm${gk===k.id?' bac':''}" onclick="(()=>{const p=P();if(!p.combatCharges)p.combatCharges={};p.combatCharges['GenieKind']='${k.id}';saveAll();render();})()">${k.icon} ${k.id}</button>`).join('')}</div>${elem?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px;display:flex;align-items:center;justify-content:space-between;gap:6px"><span>⚡ Ire du génie (niv.1) — 1×/tour : +<strong>${pb(occLvl)}</strong> dégâts <strong>${elem}</strong></span><button class="btn bsm" style="flex-shrink:0" onclick="showBanner('⚡','Ire du génie','+${pb(occLvl)} dégâts ${elem} sur une cible touchée (1×/tour)',{variant:'info'})">⚡ +${pb(occLvl)}</button></div>`:'<div style="font-size:11px;color:var(--text3);font-style:italic;margin-bottom:4px">Choisis ton type de génie ci-dessus.</div>'}<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">🍶 Catalyseur de Génie (niv.1) — Répit embouteillé : ton focaliseur sert d'abri/repos.</div>${occLvl>=6?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">🛡 Présent élémentaire (niv.6) — Résistance <strong>${elem||'(choisis ton génie)'}</strong>${elem?' (appliquée auto)':''} · Vol 9m, ${Math.max(1,chaM)}×/repos long.</div>`:''}${occLvl>=10?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">🏛 Sanctuaire du génie (niv.10) — 5 alliés dans le catalyseur (repos court amélioré).</div>`:''}${occLvl>=14?`<div style="display:flex;align-items:center;gap:6px;padding:5px 8px;background:${slU?'rgba(200,168,75,.1)':'var(--surface2)'};border:1px solid ${slU?'var(--cp)':'var(--border)'};border-radius:6px"><span class="slot-bubble${slU?' used':''}" onclick="(()=>{const p=P();if(!p.combatCharges)p.combatCharges={};p.combatCharges['SouhaitLimiteUsed']=!p.combatCharges['SouhaitLimiteUsed'];saveAll();render();})()"></span><div><div style="font-size:11px;font-weight:600">🌠 Souhait limité (niv.14) — 1×/1d4 repos longs</div><div style="font-size:10px;color:var(--text3)">Lancer 1 sort niv.≤6 sans composante</div></div><button class="btn bsm" onclick="(()=>{const p=P();if(!p.combatCharges)p.combatCharges={};delete p.combatCharges['SouhaitLimiteUsed'];saveAll();render();})()">↺</button></div>`:''}</div>`;}
