...
function renderGuerrier(p) {
  ...
  return cs('cs-guerrier',`<div class="panel mb10"><div class="pt" style="display:flex;align-items:center;gap:6px"><span class="mj-drag-handle" title="Déplacer">⠿</span>⚔ Guerrier — Capacités</div>
    ...
    ${isChampion&&guerrierLvl>=3?`<div style="margin-top:12px;border-top:1px solid var(--border);padding-top:10px"><div style="font-size:12px;font-weight:600;color:var(--cp);margin-bottom:6px">🏆 Champion</div><div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Critique amélioré (niv.3) — Coup critique sur 19-20</div>${guerrierLvl>=7?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Athlète accompli (niv.7) — ½ maîtrise aux jets FOR/DEX/CON sans maîtrise</div>`:''} ${guerrierLvl>=15?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Critique supérieur (niv.15) — Coup critique sur 18-20</div>`:''} ${guerrierLvl>=18?`<div style="padding:5px 8px;background:rgba(76,175,80,.07);border:1px solid rgba(76,175,80,.2);border-radius:6px"><div style="font-size:11px;color:var(--text2);margin-bottom:4px">Survivant (niv.18) — Si PV ≤ ½ max au début de ton tour : récupère <strong style="color:#4caf50">${5+conM}</strong> PV automatiquement.</div><button class="btn bsm" style="color:#4caf50;border-color:#4caf50" onclick="(()=>{const p=P();const heal=${5+conM};if((p.hp||0)<=Math.floor((p.hpMax||1)/2)){p.hp=Math.min(p.hpMax||0,(p.hp||0)+heal);saveAll();render();showToast('💚 Survivant : +'+heal+' PV');}else{showToast('❌ PV > ½ max — Survivant inactif');}})()">💚 Survivant (+${5+conM} PV)</button></div>`:''}</div>`:''}
    ${isMdG&&guerrierLvl>=3?(()=>{const mdgDiceMax=guerrierLvl>=15?7:guerrierLvl>=10?6:guerrierLvl>=7?5:4;const mdgDie=guerrierLvl>=18?'d12':guerrierLvl>=10?'d10':'d8';const mdgManoevres=guerrierLvl>=15?9:guerrierLvl>=10?7:guerrierLvl>=7?5:3;const mdgUsed=(p.combatCharges||{})['DésSupériorité']!==undefined?p.combatCharges['DésSupériorité']:mdgDiceMax;return`<div style="margin-top:12px;border-top:1px solid var(--border);padding-top:10px"><div style="font-size:12px;font-weight:600;color:var(--cp);margin-bottom:6px">⚔ Maître de guerre</div><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap"><div class="sb hi" style="padding:4px 12px;flex-shrink:0"><div class="sn">Dé</div><div style="font-size:16px;font-weight:700;color:var(--cp)">${mdgDie}</div></div><div class="sb" style="padding:4px 12px;flex-shrink:0"><div class="sn">Manœuvres</div><div style="font-size:16px;font-weight:700;color:var(--cp)">${mdgManoevres}</div></div></div><div style="font-size:11px;color:var(--text3);margin-bottom:4px">Supériorité martiale (DD = ${8+pb(lvl)+Math.max(forM,dexM)}) :</div><div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:4px">${Array.from({length:mdgDiceMax},(_,i)=>`<span class="slot-bubble${i<mdgUsed?'':' used'}" onclick="useCombatCharge('DésSupériorité',${mdgDiceMax})"></span>`).join('')}</div><div style="font-size:10px;color:var(--text3);margin-bottom:6px">${mdgUsed}/${mdgDiceMax} • Repos court</div><div style="display:flex;gap:4px;flex-wrap:wrap"><button class="btn bsm" onclick="rollCustomDmg('1${mdgDie}','Dé de supériorité')">🎲 1${mdgDie}</button><button class="btn bsm" onclick="recoverCombatCharge('DésSupériorité',${mdgDiceMax})">↺ Repos court</button></div>${guerrierLvl>=7?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-top:6px">Observation de l'ennemi (niv.7) — 1 min à observer : savoir si une cible est inférieure/égale/supérieure en FOR</div>`:''} ${guerrierLvl>=15?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-top:4px">Implacable (niv.15) — Si à 0 dés au début de l'initiative, récupère 1 dé de supériorité</div>`:''}<div style="margin-top:8px;border-top:1px solid var(--border);padding-top:8px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px"><div style="font-size:11px;color:var(--text3)">Manœuvres choisies (max ${mdgManoevres})</div><button class="btn bsm" onclick="openMDGManoevresModal(${mdgManoevres},'${mdgDie}',${mdgDiceMax})">⚙ Configurer</button></div>${(()=>{const cc=p.combatCharges||{};const chosen=MDG_MANOEVRES.filter(m=>cc['MDG_'+m.id]);if(!chosen.length)return '<div style="font-size:10px;color:var(--text3);font-style:italic">Aucune manœuvre configurée.</div>';const dn=parseInt(mdgDie.slice(1));const onc='(()=>{const p=P();if(!p.combatCharges)p.combatCharges={};const cur=p.combatCharges[\'DésSupériorité\']!==undefined?p.combatCharges[\'DésSupériorité\']:'+mdgDiceMax+';if(cur<1){showToast(\'❌ Plus de dés !\');return;}p.combatCharges[\'DésSupériorité\']=cur-1;const r=Math.ceil(Math.random()*'+dn+');_markUnsaved();render();showToast(\'⚔ 1'+mdgDie+' → \'+r,2500);})()';return chosen.map(m=>'<div style="padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:3px;display:flex;align-items:center;justify-content:space-between;gap:4px"><div style="flex:1"><div style="font-size:11px;font-weight:600">'+m.name+(m.tag?' <span style="font-size:10px;color:var(--cp)">'+m.tag+'</span>':'')+'</div><div style="font-size:10px;color:var(--text3)">'+m.desc+'</div></div><button class="btn bsm" style="flex-shrink:0" onclick="'+onc+'">🎲 1'+mdgDie+'</button></div>').join('');})()}</div></div>`;})():''}
    ${isCO&&guerrierLvl>=3?`<div style="margin-top:12px;border-top:1px solid var(--border);padding-top:10px"><div style="font-size:12px;font-weight:600;color:var(--cp);margin-bottom:6px">🔮 Chevalier occulte</div><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px"><div class="sb hi" style="flex:1;min-width:60px"><div class="sn">DD sorts</div><div style="font-size:16px;font-weight:700;color:var(--cp)">${8+pb(lvl)+intM}</div></div><div class="sb hi" style="flex:1;min-width:60px"><div class="sn">Atk sorts</div><div style="font-size:16px;font-weight:700;color:var(--cp)">${pb(lvl)+intM>=0?'+':''}${pb(lvl)+intM}</div></div></div><div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Sorts de magicien (INT) — sorts d'abjuration et d'évocation.</div>${(()=>{const cc=p.combatCharges||{};const liee=cc['ArmeLiee']||'';return '<div style="padding:7px 10px;background:var(--surface2);border-radius:6px;margin-bottom:4px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px"><div style="font-size:12px;font-weight:600">🔮 Lien d\'arme <span style="font-size:10px;color:var(--cp)">🔸 Action bonus</span></div><button class="btn bsm" onclick="openArmeLieeModal()">⚙ Lier</button></div>'+(liee?'<div style="font-size:11px;color:var(--text2);margin-bottom:6px">Liée : <strong>'+liee+'</strong></div><button class="btn bsm" onclick="showToast(\'🔸 Invoquer : \'+(P().combatCharges||{})[\'ArmeLiee\'],2500)">🔸 Invoquer</button>':'<div style="font-size:10px;color:var(--text3);font-style:italic">Aucune arme liée.</div>')+'</div>';})()}${guerrierLvl>=7?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Magie de guerre (niv.7) — Quand tu lances un cantrip, tu peux effectuer 1 attaque en action bonus</div>`:''} ${guerrierLvl>=10?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Frappe occulte (niv.10) — Si tu touches une créature, elle a désavantage à son prochain JS contre un de tes sorts ce tour</div>`:''} ${guerrierLvl>=15?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Charge arcanique (niv.15) — Quand tu utilises Sursaut d'action, tu peux te téléporter jusqu'à 9m</div>`:''} ${guerrierLvl>=18?`<div style="font-size:11px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px">Magie de guerre améliorée (niv.18) — L'attaque bonus s'applique à n'importe quel sort, pas seulement les cantrieps</div>`:''}</div>`:''}
  </div>`);
}
...
function openMDGManoevresModal(max, die, diceMax) {
  ...
  const rows = MDG_MANOEVRES.map(m => {
    ...
    return '<div style="display:flex;align-items:flex-start;gap:8px;padding:6px 0;border-bottom:1px solid var(--border)">' +
      ...
      '<div style="font-size:12px;font-weight:600">' + m.name + tag + '</div>' +
  ...
  openWideModal('<div style="padding:12px">' +
    ...
    '<div style="font-size:11px;color:var(--text3);margin-bottom:12px">Choisissez jusqu\'à <strong>' + max + '</strong> manœuvres (dé : 1' + die + ')</div>' +
    rows +
