...
function renderRodeur(p) {
  ...
  return cs('cs-rodeur',`<div class="panel mb10"><div class="pt" style="display:flex;align-items:center;gap:6px"><span class="mj-drag-handle" title="Déplacer">⠿</span>🏹 Rôdeur — Capacités</div>
    <div style="font-size:12px;color:var(--text3);margin-bottom:10px">DD sorts : <strong style="color:var(--cp)">${8+pb(rodeurLvl)+sagM}</strong> · Bonus attaque sort : <strong>+${pb(rodeurLvl)+sagM}</strong></div>
    ${combatStyle?`<div style="margin-bottom:8px;padding:6px 8px;background:var(--surface2);border-radius:6px;font-size:12px;color:var(--text2)">⚔ Style de combat : <strong>${combatStyle}</strong></div>`:''}
    ${rodeurLvl>=3?`<div style="margin-bottom:8px;padding:6px 8px;background:var(--surface2);border-radius:6px;font-size:12px;color:var(--text2)">👁 Vigilance primitive (niv.3) — Action + emplacement : détecter aberrations/célestes/dragons/élémentaires/fées/fiélons/morts-vivants à 1,5 km (1 min/niveau d'emplacement)</div>`:''}
    ${rodeurLvl>=5?`<div style="margin-bottom:8px;padding:6px 8px;background:var(--surface2);border-radius:6px;font-size:12px;color:var(--text2)">⚔ Attaque supplémentaire (niv.5) — Attaquer 2× lors de l'action Attaquer</div>`:''}
    ${rodeurLvl>=8?`<div style="margin-bottom:8px;padding:6px 8px;background:var(--surface2);border-radius:6px;font-size:12px;color:var(--text2)">🚶 Foulée tellurique (niv.8) — Terrains difficiles non magiques sans surcoût de mouvement. Avantage JS contre enchevêtrement et plantes magiques</div>`:''}
    ${rodeurLvl>=10?`<div style="margin-bottom:8px;padding:6px 8px;background:var(--surface2);border-radius:6px;font-size:12px;color:var(--text2)">🌿 Camouflage naturel (niv.10) — 1 min de préparation avec matériaux naturels : +10 à Discrétion si immobile contre une surface solide</div>`:''}
    ${rodeurLvl>=14?`<div style="margin-bottom:8px;padding:6px 8px;background:var(--surface2);border-radius:6px;font-size:12px;color:var(--text2)">🔸 👻 Disparition (niv.14) — Se cacher en action bonus. Ne peut pas être suivi par des moyens non magiques</div>`:''}
    ${rodeurLvl>=18?`<div style="margin-bottom:8px;padding:6px 8px;background:var(--surface2);border-radius:6px;font-size:12px;color:var(--text2)">👂 Sens sauvages (niv.18) — Pas de désavantage pour attaquer une créature invisible à portée. Localise les créatures invisibles à 9m</div>`:''}
    ${rodeurLvl>=20?`<div style="margin-bottom:8px;padding:6px 8px;background:rgba(200,168,75,.07);border:1px solid rgba(200,168,75,.3);border-radius:6px;font-size:12px;color:var(--text2)">🎯 Tueur implacable (niv.20) — 1×/tour : +${sagM} aux jets d'attaque ou de dégâts contre un ennemi juré</div>`:''}
    ${isChasseur&&rodeurLvl>=3?`<div style="border-top:1px solid var(--border);padding-top:10px;margin-top:8px"><div style="font-size:13px;font-weight:600;color:var(--cp);margin-bottom:6px">🎯 Chasseur</div><div style="font-size:12px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Proie du chasseur (niv.3) — Tueur de colosses (+1d8 si cible sous max PV) / Tueur de géants (réaction contre grande créature) / Briseur de hordes (attaque bonus contre cible adjacente)</div>${rodeurLvl>=7?`<div style="font-size:12px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Tactiques défensives (niv.7) — Échapper à la horde (désav. AO) / Défense multi-attaques (+4 CA contre même cible ce tour) / Moral d'acier (avantage JS Peur)</div>`:''} ${rodeurLvl>=11?`<div style="font-size:12px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Attaques multiples (niv.11) — Volée (tir sur N cibles dans 3m d'un point ciblé) / Attaque tourbillonnante (corps à corps sur toutes créatures à 1,5m)</div>`:''} ${rodeurLvl>=15?`<div style="font-size:12px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px">Défense supérieure (niv.15) — Esquive totale / Retour de bâton (réaction : retourner l'attaque manquée vers une autre créature) / Esquive instinctive (réaction : dégâts de moitié)</div>`:''}</div>`:''}
    ${isMdB&&rodeurLvl>=3?`<div style="border-top:1px solid var(--border);padding-top:10px;margin-top:8px"><div style="font-size:13px;font-weight:600;color:var(--cp);margin-bottom:6px">🐾 Maître des bêtes</div><div style="font-size:12px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Compagnon du rôdeur (niv.3) — Bête taille M max, CR ≤ 1/4. PV = 4 × niveau Rôdeur. Agit à ton tour sur ordre.</div>${rodeurLvl>=7?`<div style="font-size:12px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">🔸 Entraînement exceptionnel (niv.7) — Action bonus : commander Aider/Foncer/Désengager. Attaques du compagnon = magiques.</div>`:''} ${rodeurLvl>=11?`<div style="font-size:12px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Fureur bestiale (niv.11) — Sur ordre Attaquer : le compagnon peut attaquer 2×.</div>`:''} ${rodeurLvl>=15?`<div style="font-size:12px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px">Partage des sorts (niv.15) — Un sort te ciblant affecte aussi le compagnon s'il est à 9m ou moins.</div>`:''}</div>`:''}
  </div>`);
}
...
function renderCompagnonAnimal(p) {
  ...
  return cs('cs-beast-companion',`<div class="panel mb10" style="border-color:rgba(200,168,75,.35)"><div class="pt" style="display:flex;align-items:center;justify-content:space-between"><div style="display:flex;align-items:center;gap:6px"><span class="mj-drag-handle" title="Déplacer">⠿</span><span>${bc.icon} Compagnon — ${bc.name}</span></div><div style="display:flex;gap:6px"><button class="btn bsm" onclick="openBeastCompanionModal()" title="Changer">⚙</button><button class="btn bsm bdanger" onclick="removeBeastCompanion()">✕ Révoquer</button></div></div>
      <div class="g3 mb6">
        ...
        <div class="sb"><div class="sn">Vitesse</div><div style="font-size:11px;font-weight:600;line-height:1.3">${bc.speed}</div></div>
      </div>
      ...
      <div class="g6 mb8">${ABILITIES_SH.map((ab,i)=>`<div class="sb" style="padding:3px"><div class="sn" style="font-size:9px">${ab}</div><div style="font-size:13px;font-weight:600">${bc.ab[i]}</div><div class="sm" style="font-size:9px">${fmt(Math.floor((bc.ab[i]-10)/2))}</div></div>`).join('')}</div>
      ...
      ${bc.traits&&bc.traits.length?`<div><div class="fl mb4">Traits</div>${bc.traits.map(t=>`<div style="font-size:12px;color:var(--text2);padding:2px 0">▹ ${t}</div>`).join('')}</div>`:''}
    </div>`);
}
...
function bcShowDetail(idx, hpMax) {
  ...
  openWideModal(`<div class="pt">🐾 ${b.name}</div>
    ...
    ${b.attacks.map(a=>`<div style="padding:6px;background:var(--surface2);border-radius:6px;margin-bottom:4px;font-size:13px">
      <strong>${a.name}</strong> : +${a.bonus} att. · <strong>${a.dmg}</strong> ${a.type}${a.special?`<div style="font-size:11px;color:var(--text3)">${a.special}</div>`:''}
    </div>`).join('')}
    ${b.traits.length?`<div style="font-size:12px;color:var(--text3);margin-top:8px">${b.traits.map(t=>`▹ ${t}`).join('<br>')}</div>`:''}
