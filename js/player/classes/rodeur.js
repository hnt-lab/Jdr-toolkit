...
function renderGardienDrake(p, rodeurLvl) {
  ...
  let html = `<div style="border-top:1px solid var(--border);padding-top:10px;margin-top:8px"><div style="font-size:18px;font-weight:600;color:var(--cp);margin-bottom:6px">🐉 Gardien de drake</div>
    <div style="font-size:17px;color:var(--text2);padding:5px 8px;background:var(--surface2);border-radius:6px;margin-bottom:4px">Don draconique (niv.3) — Tu connais Thaumaturgie (sort de rôdeur pour toi) et tu parles, lis et écris le draconique (ou une autre langue au choix).</div>`;
  if (!dk || !dk.active) {
    html += `<div style="padding:7px 10px;background:var(--surface2);border-radius:6px;margin-bottom:4px">
      <div style="font-size:17px;color:var(--text3);margin-bottom:6px">Action : invoque ton drake (${hpMax} PV, CA ${14+pbv}, taille ${taille}). Après une invocation, il faut un repos long ou dépenser un emplacement de sort de niv.1+ pour le réinvoquer. Choisis son essence draconique :</div>
    ...
    const dcol = dpct>50?'#4caf50':dpct>25?'#ff9800':'#e53935';
    html += `<div style="padding:7px 10px;background:rgba(255,87,34,.06);border:1px solid rgba(255,87,34,.25);border-radius:6px;margin-bottom:4px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><span style="font-size:18px;font-weight:600">🐉 Drake — essence ${esc(dk.essence)} (taille ${taille})</span><button class="btn bsm bdanger" onclick="dismissDrake()">✕ Renvoyer</button></div>
      <div style="display:flex;gap:8px;font-size:17px;color:var(--text3);margin-bottom:6px;flex-wrap:wrap"><span>CA <strong>${14+pbv}</strong></span><span>PV <strong style="color:${dcol}">${dk.hpCur}/${dk.hpMax}</strong></span><span>Vitesse 12m${rodeurLvl>=7?' + vol 12m':''}</span><span>Immunité ${esc(dk.essence)}</span></div>
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:6px">
        ...
        <button class="btn bsm" style="background:#b71c1c;color:#fff;border-color:#b71c1c" onclick="openDrakeHpModal()">💥 / 💚 PV</button>
      </div>
      <div style="font-size:15px;color:var(--text3)">↪ Coups imprégnés (réaction) : quand une créature à 9m du drake touche avec une arme, l'attaque inflige +1d6 dégâts de ${esc(dk.essence)}. <button class="btn bsm" onclick="rollCustomDmg('1d6','Coups imprégnés')">🎲 1d6</button></div>
      ${rodeurLvl>=7?`<div style="font-size:15px;color:var(--text3);margin-top:4px">🛡 Lien du croc et d'écailles (niv.7) : tu as la résistance aux dégâts de ${esc(dk.essence)} tant que le drake est invoqué${rodeurLvl>=15?' ; monture volante possible (taille G)':' ; monture possible (pas de vol monté)'}.</div>`:''}
    </div>`;
  }
  if (rodeurLvl>=11) {
    html += `<div style="padding:7px 10px;background:var(--surface2);border-radius:6px;margin-bottom:4px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px"><span style="font-size:18px;font-weight:600">🌬 Souffle de drake <span style="font-size:15px;color:var(--text3)">niv.11</span></span><span class="slot-bubble${souffleUsed?' used':''}" onclick="(()=>{const p=P();if(!p.combatCharges)p.combatCharges={};p.combatCharges['SouffleDrake']=!p.combatCharges['SouffleDrake'];saveAll();render();})()"></span></div>
      <div style="font-size:17px;color:var(--text3);margin-bottom:6px">Action : cône de 9m — JS DEX DD ${8+pbv+sagM} ou <strong>${souffleDice}</strong> dégâts (acide, froid, feu, foudre ou poison au choix), moitié si réussi. 1×/repos long, ou dépense d'un emplacement de sort de niv.3+.</div>
...
function openDrakeHpModal() {
  ...
  openModal(`<div class="pt">🐉 PV — Drake</div>
    ...
    <div style="display:flex;gap:8px">
      <button class="btn" style="flex:1;background:#b71c1c;color:#fff;border-color:#b71c1c" onclick="_drakeApplyHp(-1)">💥 Dégâts</button>
      <button class="btn" style="flex:1;background:#2e7d32;color:#fff;border-color:#2e7d32" onclick="_drakeApplyHp(1)">💚 Soins</button>
    </div>`);
...
function renderCompagnonAnimal(p) {
  ...
  return cs('cs-beast-companion',`<div class="panel mb10" style="border-color:rgba(200,168,75,.35)"><div class="pt" style="display:flex;align-items:center;justify-content:space-between"><div style="display:flex;align-items:center;gap:6px"><span class="mj-drag-handle" title="Déplacer">⠿</span><span>${bc.icon} Compagnon — ${bc.name}</span></div><div style="display:flex;gap:6px"><button class="btn bsm" onclick="openBeastCompanionModal()" title="Changer">⚙</button><button class="btn bsm bdanger" onclick="removeBeastCompanion()">✕ Révoquer</button></div></div>
      <div class="g3 mb6">
        ...
        <div class="sb"><div class="sn">CA</div><div style="font-size:25px;font-weight:700">${bc.ac+_bcPb}</div><div class="sm">${bc.ac}+${_bcPb} maîtrise</div></div>
