...
function renderDruide(p) {
  ...
  const crMax = isLune?(druLvl>=6?String(Math.floor(druLvl/3)):'1'):(druLvl>=8?'1':druLvl>=4?'1/2':'1/4');
...
function openWildshapeModal() {
  ...
  const crMax = _isLune?(druLvl>=6?Math.floor(druLvl/3):1):(druLvl>=8?1:druLvl>=4?0.5:0.25);
  const canSwim = druLvl>=4;
  const canFly = druLvl>=8;
  const fsMax = druLvl>=20?99:2;
...
function enterElementalForm(efIdx) {
  ...
  const fsUsed = (p.combatCharges['Forme sauvage']!==undefined)?p.combatCharges['Forme sauvage']:fsMax;
  if (druLvl<20&&fsUsed<2) { showToast('❌ Il faut 2 utilisations de Forme sauvage disponibles !'); return; }
  if (druLvl<20) p.combatCharges['Forme sauvage'] = fsUsed-2;
  delete p.combatCharges['FormeElementaire'];
