...
function renderClerc(p) {
  ...
  if (clercLvl >= 2) {
    ...
    panels.push(`<div style="margin-bottom:10px;padding:8px;background:var(--surface2);border-radius:2px">
      <div style="font-size:13px;font-weight:600;color:var(--cp);margin-bottom:6px">✝ Conduit divin</div>
      <div style="font-size:13px;color:var(--text3);margin-bottom:6px">Renvoi des morts-vivants (action) + capacité de domaine. Récup. repos court.</div>
      ...
      <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
        ...
        <span style="font-size:12px;color:var(--text3)">${cdUsed}/${cdMax}</span>
      </div>
    </div>`);
  }
  ...
  if (clercLvl >= 5) {
    ...
    panels.push(`<div style="margin-bottom:10px;padding:8px;background:var(--surface2);border-radius:2px">
      <div style="font-size:13px;font-weight:600;color:var(--cp);margin-bottom:2px">💀 Destruction des morts-vivants</div>
      <div style="font-size:13px;color:var(--text3)">Renvoi des morts-vivants : créatures de FP ≤ <strong>${fp}</strong> sont détruites automatiquement si elles ratent leur JS.</div>
    </div>`);
  }
  ...
  if (clercLvl >= 10) {
    ...
    panels.push(`<div style="margin-bottom:10px;padding:8px;background:${idUsed?'var(--surface2)':'rgba(200,168,75,.06)'};border:1px solid ${idUsed?'var(--border)':'rgba(200,168,75,.4)'};border-radius:2px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
        <span style="font-size:13px;font-weight:600;color:var(--cp)">🙏 Intervention divine</span>
      ...
      <div style="font-size:13px;color:var(--text3)">${isLvl20?'Niv.20 — Réussit automatiquement, aucun jet nécessaire.':idUsed?'Utilisé — Recharge dans 7 jours ou repos long.':`Action : lancer 1d100. Si résultat ≤ ${clercLvl} → divinité intervient (effet du MJ). Recharge : 7 jours si succès, repos long si échec. <button class="btn bsm" style="font-size:12px;margin-top:4px" onclick="(()=>{const r=Math.ceil(Math.random()*100);showToast('🙏 Intervention divine : d100 = <strong>'+r+'</strong> '+(r<=${clercLvl}?' ✅ Succès !':' ❌ Échec'))})()">🎲 d100</button>`}</div>
    </div>`);
  }
  ...
  if (clercPath) {
    const domStyle = {
      'Domaine de la vie':    {bg:'rgba(76,175,80,.07)',  border:'rgba(76,175,80,.25)',  col:'var(--good)', icon:'💚'},
      'Domaine de la lumière':{bg:'rgba(255,193,7,.07)',  border:'rgba(255,193,7,.4)',   col:'var(--cp)', icon:'☀'},
      'Domaine de la nature': {bg:'rgba(76,175,80,.07)',  border:'rgba(76,175,80,.25)',  col:'var(--good)', icon:'🌿'},
      'Domaine de la tempête':{bg:'rgba(33,150,243,.07)', border:'rgba(33,150,243,.25)', col:'var(--info)', icon:'⚡'},
      'Domaine de la duperie':{bg:'rgba(156,39,176,.07)',border:'rgba(156,39,176,.25)',col:'var(--arcane)', icon:'🎭'},
      'Domaine de la guerre': {bg:'rgba(244,67,54,.07)',  border:'rgba(244,67,54,.25)',  col:'#f44336', icon:'⚔'},
      'Domaine du savoir':    {bg:'rgba(33,150,243,.07)', border:'rgba(33,150,243,.25)', col:'var(--info)', icon:'📚'},
    ...
    if (dn === 'Domaine de la vie') {
      dc += `<div style="padding:7px 10px;background:var(--surface2);border-radius:2px;margin-bottom:6px">
        <div style="font-size:13px;font-weight:600;margin-bottom:4px">💚 Disciple de la vie <span style="font-size:12px;color:var(--text3);margin-left:4px">niv.1</span></div>
        <div style="font-size:13px;color:var(--text3);margin-bottom:6px">Sorts de soin niv.1+ : <strong>+2 + niveau du sort</strong> PV supplémentaires à la cible. Quel niveau venez-vous de lancer ?</div>
      ...
      if (clercLvl>=8)  dc += _clercPanel(`⚡ Frappe divine`,'niv.8','',`1×/tour, quand tu touches avec une arme : <strong>+${clercLvl>=14?'2d8':'1d8'} radiants</strong>. <button class="btn bsm" style="font-size:12px;margin-top:4px" onclick="rollCustomDmg('${clercLvl>=14?'2d8':'1d8'}','Frappe divine')">🎲 ${clercLvl>=14?'2d8':'1d8'}</button>`);
    ...
    // ─── LUMIÈRE ──────────────────────────────────────────────
      ...
      dc += `<div style="padding:7px 10px;background:var(--surface2);border-radius:2px;margin-bottom:6px">
        <div style="font-size:13px;font-weight:600;margin-bottom:4px">🔆 Illumination protectrice <span style="font-size:12px;color:var(--cp);margin-left:4px">↪ Réaction</span> <span style="font-size:12px;color:var(--text3);margin-left:4px">niv.1</span></div>
        <div style="font-size:13px;color:var(--text3);margin-bottom:6px">${clercLvl>=6?'Quand toi ou un allié à 9m est attaqué par une créature visible':'Quand tu es attaqué par une créature à 9m'} : imposer désavantage au jet d'attaque. Immunisé si l'attaquant ne peut pas être aveuglé.</div>
        ...
        <div style="display:flex;gap:6px;align-items:center">
          ...
          <span style="font-size:12px;color:var(--text3)">${ipCur}/${ipMax}</span>
        </div>
      </div>`;
      dc += _clercPanel('☀ Radiance de l\'aube','Conduit divin','',`Action : dissiper ténèbres magiques à 9m. Créatures hostiles à 9m → JS CON DD ${ddSorts} ou <strong>2d10+${clercLvl} radiants</strong> (moitié si réussi). <button class="btn bsm" style="font-size:12px;margin-top:4px" onclick="rollCustomDmg('2d10','Radiance de l\\'aube')">🎲 2d10</button>`);
    ...
    // ─── NATURE ───────────────────────────────────────────────
      ...
      if (clercLvl>=8) dc += _clercPanel(`⚡ Frappe divine`,'niv.8','',`1×/tour, quand tu touches avec une arme : <strong>+${clercLvl>=14?'2d8':'1d8'} froid, feu ou foudre</strong> (au choix). <button class="btn bsm" style="font-size:12px;margin-top:4px" onclick="rollCustomDmg('${clercLvl>=14?'2d8':'1d8'}','Frappe divine')">🎲 ${clercLvl>=14?'2d8':'1d8'}</button>`);
    ...
    // ─── TEMPÊTE ──────────────────────────────────────────────
      ...
      dc += `<div style="padding:7px 10px;background:var(--surface2);border-radius:2px;margin-bottom:6px">
        <div style="font-size:13px;font-weight:600;margin-bottom:4px">⚡ Fureur de l'ouragan <span style="font-size:12px;color:var(--cp);margin-left:4px">↪ Réaction</span> <span style="font-size:12px;color:var(--text3);margin-left:4px">niv.1</span></div>
        <div style="font-size:13px;color:var(--text3);margin-bottom:6px">Quand une créature à 1,5m te touche : JS DEX DD ${ddSorts} ou <strong>2d8 foudre ou tonnerre</strong> (moitié si réussi). Récup. repos long.</div>
        ...
        <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
          ...
          <span style="font-size:12px;color:var(--text3)">${foCur}/${foMax}</span>
      ...
      if (clercLvl>=8) dc += _clercPanel(`⚡ Frappe divine`,'niv.8','',`1×/tour, quand tu touches avec une arme : <strong>+${clercLvl>=14?'2d8':'1d8'} tonnerre</strong>. <button class="btn bsm" style="font-size:12px;margin-top:4px" onclick="rollCustomDmg('${clercLvl>=14?'2d8':'1d8'}','Frappe divine')">🎲 ${clercLvl>=14?'2d8':'1d8'}</button>`);
    ...
    // ─── TROMPERIE ────────────────────────────────────────────
      ...
      if (clercLvl>=8) dc += _clercPanel(`⚡ Frappe divine`,'niv.8','',`1×/tour, quand tu touches avec une arme : <strong>+${clercLvl>=14?'2d8':'1d8'} poison</strong>. <button class="btn bsm" style="font-size:12px;margin-top:4px" onclick="rollCustomDmg('${clercLvl>=14?'2d8':'1d8'}','Frappe divine')">🎲 ${clercLvl>=14?'2d8':'1d8'}</button>`);
    ...
    // ─── GUERRE ───────────────────────────────────────────────
      ...
      dc += `<div style="padding:7px 10px;background:var(--surface2);border-radius:2px;margin-bottom:6px">
        <div style="font-size:13px;font-weight:600;margin-bottom:4px">⚔ Prêtre de guerre <span style="font-size:12px;color:var(--text3);margin-left:4px">niv.1</span></div>
        <div style="font-size:13px;color:var(--text3);margin-bottom:6px">Quand tu utilises l'action Attaquer : faire une attaque avec une arme en action bonus. ${pgMax}×/repos long.</div>
        ...
        <div style="display:flex;gap:6px;align-items:center">
          ...
          <span style="font-size:12px;color:var(--text3)">${pgCur}/${pgMax}</span>
        </div>
      </div>`;
      if (clercLvl>=2) dc += `<div style="padding:7px 10px;background:var(--surface2);border-radius:2px;margin-bottom:6px"><div style="font-size:13px;font-weight:600;margin-bottom:2px">🎯 Frappe guidée <span style="font-size:12px;color:var(--text3);margin-left:4px">Conduit divin</span></div><div style="font-size:13px;color:var(--text3);margin-bottom:6px">Après ton jet d’attaque (avant l’annonce du MJ) : +<strong>10</strong>. Coût : 1 conduit.</div><button class="btn bsm bac" onclick="_clercGuidedStrike('Frappe guidée','+10 à ton jet d’attaque')">🎯 +10 (−1 conduit)</button></div>`;
      if (clercLvl>=6) dc += `<div style="padding:7px 10px;background:var(--surface2);border-radius:2px;margin-bottom:6px"><div style="font-size:13px;font-weight:600;margin-bottom:2px">🛡 Bénédiction du dieu de la guerre <span style="font-size:12px;color:var(--cp);margin-left:4px">↪ Réaction</span> <span style="font-size:12px;color:var(--text3);margin-left:4px">niv.6</span></div><div style="font-size:13px;color:var(--text3);margin-bottom:6px">Réaction : une créature à 9 m fait un jet d’attaque → +<strong>10</strong>. Coût : 1 conduit.</div><button class="btn bsm bac" onclick="_clercGuidedStrike('Bénédiction du dieu de la guerre','+10 au jet d’attaque d’un allié à 9 m')">🛡 +10 allié (−1 conduit)</button></div>`;
      if (clercLvl>=8) dc += _clercPanel(`⚡ Frappe divine`,'niv.8','',`1×/tour, quand tu touches avec une arme : <strong>+${clercLvl>=14?'2d8':'1d8'} du même type que l'arme</strong>. <button class="btn bsm" style="font-size:12px;margin-top:4px" onclick="rollCustomDmg('${clercLvl>=14?'2d8':'1d8'}','Frappe divine')">🎲 ${clercLvl>=14?'2d8':'1d8'}</button>`);
    ...
    // ─── SAVOIR ───────────────────────────────────────────────
      ...
      if (clercLvl>=17) {
        ...
        dc += `<div style="padding:7px 10px;background:${vpUsed?'rgba(33,150,243,.08)':'var(--surface2)'};border:1px solid ${vpUsed?'rgba(33,150,243,.4)':'var(--border)'};border-radius:2px">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
            <span style="font-size:13px;font-weight:600">🔮 Visions du passé <span style="font-size:12px;font-weight:400;color:var(--text3);margin-left:4px">niv.17</span></span>
          ...
          <div style="font-size:13px;color:var(--text3)">${vpUsed?'Utilisé — Recharge après un repos court.':'Méditation 1 min : visions d\'un objet (propriétaires passés) ou d\'une zone (jusqu\'à SAG jours dans le passé). Conc. nécessaire.'}</div>
        </div>`;
      }
    }
    ...
    // ─── FORGE ────────────────────────────────────────────────
      ...
      if (clercLvl>=8) dc += _clercPanel(`⚡ Frappe divine`,'niv.8','',`1×/tour, quand tu touches avec une arme : <strong>+${clercLvl>=14?'2d8':'1d8'} feu</strong>. <button class="btn bsm" style="font-size:12px;margin-top:4px" onclick="rollCustomDmg('${clercLvl>=14?'2d8':'1d8'}','Frappe divine')">🎲 ${clercLvl>=14?'2d8':'1d8'}</button>`);
    ...
    panels.push(`<div style="margin-top:6px;padding:10px;background:${ds.bg};border:1px solid ${ds.border};border-radius:2px">
      <div style="font-size:13px;font-weight:700;color:${ds.col};margin-bottom:8px">${ds.icon} ${esc(dn)}</div>
      ${dc}
    </div>`);
  }
...
function _clercPanel(title, tag, extra, desc) {
  return `<div style="padding:7px 10px;background:var(--surface2);border-radius:2px;margin-bottom:6px">
    <div style="font-size:13px;font-weight:600;margin-bottom:2px">${title}${tag?` <span style="font-size:12px;color:var(--text3);margin-left:4px">${tag}</span>`:''}${extra}</div>
    <div style="font-size:13px;color:var(--text3)">${desc}</div>
  </div>`;
}
