...
function tabCompetences(p){
  ...
  // Nombre max de compÃ©tences autorisÃ©es
  ...
  <div><div class="panel mb10">
    <div class="pt" style="display:flex;align-items:center;justify-content:space-between"><span>CompÃ©tences</span><span style="font-size:11px;color:var(--text3)">${currentCount}/${maxSkills} maÃ®trisÃ©es</span></div>
    ${SKILLS.map(sk=>{
      ...
      return`<div class="sk" style="${classSkill?`color:var(--cp)`:''};${isLocked?'cursor:default':'cursor:pointer'}" onclick="${isLocked?'':'cycleSkillIfAllowed(\''+sk.name+'\','+prof+','+maxSkills+')'}">
        <span class="sk-dot${dotClass}" title="${prof===2?'Expertise (Ã—2)':prof===1?'MaÃ®trise':hasToucheATout?'Touche-Ã -tout (Â½ maÃ®trise)':'Aucune maÃ®trise'}"></span>
  ...
  <div>
    <div class="panel mb10"><div class="pt" style="display:flex;align-items:center;justify-content:space-between"><span>CapacitÃ©s & traits</span>${isMJ()?`<button class="btn bsm" onclick="openFeatSearch()">+ Ajouter</button>`:''}</div>
      ${(()=>{
        // Noms de capacitÃ©s purement de combat (ont un tracker ou charges dans combatFeatures)
        ...
        const displayFeats=(p.features||[]).filter(f=>{
          ...
          if(f.name.startsWith('Sorts du cercle')||f.name.startsWith('CapacitÃ© du cercle'))return false;
          if(isFeatExcluded(f.name))return false;
          if(f.name.includes('(choix)')&&f.classe&&chosenArchByClass[f.classe])return false;
          return true;
        });
        if(!displayFeats.length)return`<div style="font-size:12px;color:var(--text3);font-style:italic">Aucune capacitÃ© passive.</div>`;
        ...
        const fsVolNage=druLvl>=8?'Vol âœ“ / Nage âœ“':druLvl>=4?'Nage âœ“ / Vol âœ—':'Nage âœ— / Vol âœ—';
        ...
        return displayFeats.map((f,idx)=>{
          ...
          return`<div class="sort-row">
            <div class="sort-head" onclick="document.getElementById('${fid}').classList.toggle('open')">
              ...
              <span style="color:var(--text3);font-size:11px;margin-right:8px">â–¾</span>
              ${isMJ()?`<span onclick="event.stopPropagation();removeFeat(${realIdx})" style="cursor:pointer;color:var(--text3);font-size:15px">Ã—</span>`:''}
            </div>
            <div class="sort-body" id="${fid}">
              <p>${esc(desc||'Consulter aidedd.org/regles/classes/ pour les dÃ©tails.')}</p>
              ${isFormeSauvage&&druLvl>=2?`<div style="margin-top:8px;padding:6px 8px;background:var(--surface3);border-radius:6px;font-size:12px">
                <div style="color:#4caf50;margin-bottom:4px">ðŸº CR max actuel : <strong>${fsCrMax}</strong> â€” ${fsVolNage}</div>
                <div style="display:flex;align-items:center;gap:8px">
                  ...
                  <span style="color:var(--text3)">${fsUsed}/2 â€¢ Repos court</span>
                  <button class="btn bsm" onclick="recoverCombatCharge('Forme sauvage',2)">â†º</button>
                </div>
              </div>`:''}
            </div>
          </div>`;
    ...
    ${rd?`<div class="panel"><div class="pt">Traits raciaux</div><div style="font-size:13px;color:var(--text2);line-height:1.6">${esc(rd.traits)}</div><div style="font-size:11px;color:var(--text2);margin-top:8px"><span style="color:var(--text3)">Vitesse :</span> ${rd.speed}m â€¢ <span style="color:var(--text3)">Langues :</span> ${esc(rd.languages)}</div></div>`:''}
  </div></div>`;
}
...
function cycleSkillIfAllowed(name,current,maxSkills){
  ...
  if(!isMJ()&&current===0&&total>=maxSkills){
    showToast('âŒ Maximum de compÃ©tences atteint.');return;
  }
  const nextVal=((p.skillProf[name]||0)+1)%3;
  if(!isMJ()&&nextVal===2){
    const maxExp=maxExpertise(p);
    if(maxExp===0){showToast('âŒ Ta classe ne permet pas l\'expertise (Roublard ou Barde requis).');return;}
    const curExp=Object.values(p.skillProf).filter(v=>v===2).length;
    if(curExp>=maxExp){showToast(`âŒ Limite d'expertise atteinte (${maxExp} max pour ta classe).`);return;}
  }
  p.skillProf[name]=nextVal;
  render();
}
function openFeatSearch(){openModal(`<div class="pt">Ajouter une capacitÃ©</div>
<input class="fi" id="featQ" placeholder="ðŸ” Chercher dans les capacitÃ©s SRD..." oninput="filterFeats(this.value)" style="margin-bottom:8px">
...
<div style="margin-top:12px;padding:10px;background:var(--surface2);border-radius:8px;border:1px solid var(--border)">
  <div style="font-size:12px;font-weight:600;color:var(--cp);margin-bottom:8px">âœ¨ CapacitÃ© personnalisÃ©e (univers alternatif)</div>
  <input class="fi" id="featCustomName" placeholder="Nom de la capacitÃ©" style="margin-bottom:6px">
  <textarea class="fi" id="featCustomDesc" rows="2" placeholder="Description de la capacitÃ©..." style="resize:vertical;margin-bottom:6px"></textarea>
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:8px">
    ...
    <div><div style="font-size:10px;color:var(--text3);margin-bottom:3px">RÃ©cupÃ©ration</div><select class="fi" id="featCustomRecovery"><option value="passive">Passif</option><option value="short">Repos court</option><option value="long">Repos long</option></select></div>
    <div><div style="font-size:10px;color:var(--text3);margin-bottom:3px">DÃ© (ex : 2d6+3)</div><input class="fi" id="featCustomDice" placeholder="2d6+3"></div>
  </div>
  <button class="btn bac bsm" style="width:100%" onclick="addCustomCombatFeat()">+ Ajouter cette capacitÃ©</button>
</div>
<button class="btn bsm bdanger" style="margin-top:8px;width:100%" onclick="closeModal()">Fermer</button>`);}
const FEATS_SRD=[
  {name:"Rage",classe:"Barbare",desc:"2/repos long. Bonus dÃ©gÃ¢ts, rÃ©sistance physique, avantage FOR."},
  {name:"Attaque supplÃ©mentaire",classe:"Guerrier/Barbare",desc:"Attaque 2 fois par action Attaquer au niv.5."},
  {name:"Frappe divine",classe:"Paladin",desc:"DÃ©pense un emplacement pour ajouter des dÃ©gÃ¢ts radiants."},
  {name:"Roublardise",classe:"Roublard",desc:"1d6 dÃ©gÃ¢ts supp. si avantage ou alliÃ© adjacent."},
  {name:"Terreur sombre",classe:"Occultiste",desc:"MÃ©cÃ¨ne surnaturel. Emplacements rÃ©cupÃ©rÃ©s au repos court."},
  {name:"DÃ©fense sans armure",classe:"Barbare/Moine",desc:"CA calculÃ©e diffÃ©remment sans armure."},
  {name:"Conduit divin",classe:"Clerc/Paladin",desc:"Effets cumulables, pas les utilisations."},
  {name:"MÃ©tamagie",classe:"Ensorceleur",desc:"DÃ©pense des points de sorcellerie pour modifier tes sorts."},
];
function renderFeatList(q){return FEATS_SRD.filter(f=>!q||f.name.toLowerCase().includes(q.toLowerCase())).map(f=>`<div class="aci" onclick="addFeat('${esc(f.name)}','${esc(f.desc)}','${esc(f.classe)}')"><div class="ain">${esc(f.name)}</div><div class="ais">${esc(f.desc.slice(0,80))}â€¦</div></div>`).join('')||'<div style="color:var(--text3);font-size:12px;padding:8px">Aucun rÃ©sultat.</div>';}
...
function addCustomCombatFeat(){
  ...
  if(!n){showToast('âŒ Nom requis.');return;}
  ...
  const feat={name:n,desc:d,charges:ch,recovery:ch>0?rec:'passive',icon:'âš¡'};
