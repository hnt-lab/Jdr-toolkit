...
function tabSac(p){
  ...
  const invRows=inv.length?_invSorted.map(({item,i})=>{const _t=_itemType(item.name,item);const _ico=_TYPE_ICON[_t]||'ðŸ“¦';return`<div class="inv-item" style="flex-direction:column;align-items:flex-start;gap:4px;${(item.qty||0)===0?'opacity:.6':''}">
    <div style="display:flex;align-items:center;gap:8px;width:100%">
      <span style="font-size:15px;flex-shrink:0" title="${_t||'Objet'}">${_ico}</span>
      <span style="flex:1;font-size:13px;font-weight:600">${esc(item.name)}${item.magic?` <span class="magic-badge">âœ¨${item.linkedTo?' '+esc(item.linkedTo):''}</span>`:''} ${(item.qty||0)===0?'<span style="font-size:10px;color:#e53935;font-weight:600;border:1px solid rgba(229,57,53,.4);border-radius:4px;padding:0 4px">Ã‰puisÃ©</span>':''}</span>
      <button class="btn bsm" style="padding:1px 6px;font-size:16px;line-height:1" onclick="adjustQty(${i},-1)">âˆ’</button>
      ...
      <span onclick="removeInvItem(${i})" style="cursor:pointer;color:var(--text3);font-size:15px;margin-left:4px">Ã—</span>
    </div>
    ...
    ${(_t==='P'||_t==='SC')&&(item.qty||0)>0?`<div style="margin-top:4px"><button class="btn bsm" style="color:#7986cb;border-color:rgba(121,134,203,.4)" onclick="useConsumable(${i})">${_t==='P'?'ðŸ§ª Utiliser':'ðŸ“œ Lancer'}</button></div>`:''}
    ${item.attunement?`<div style="margin-top:4px"><button class="btn bsm" onclick="toggleAttunement(${i})" style="color:${item.attuned?'#e53935':'#9c27b0'};border-color:${item.attuned?'rgba(229,57,53,.4)':'rgba(156,39,176,.4)'}">${item.attuned?'ðŸ”“ Rompre le lien':'ðŸ”— Se lier'}</button>${item.attuned?`<span style="font-size:10px;color:#9c27b0;margin-left:6px">LiÃ© âœ“</span>`:''}</div>`:''}
  ...
  <div class="panel mb10">
    <div class="pt" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
      <span>ðŸª™ Bourse</span>
      <span style="font-size:10px;color:var(--text3);font-style:italic">Conversion automatique</span>
    </div>
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:6px">
      ${[['pc','Cuivre','#b87333'],['pa','Argent','#b0bec5'],['pe','Ã‰lectrum','#6abfad'],['po','Or','var(--cp)'],['pp','Platine','#dcdcdc']].map(([k,lbl,col])=>`<div class="coin"><div class="coin-lbl" style="color:${col}">${lbl}</div><div id="bourse_${k}" class="coin-val" style="font-size:18px;font-weight:600;text-align:center;padding:4px 0;color:var(--text)">${cur[k]||0}</div></div>`).join('')}
    </div>
    <div style="margin-top:8px;font-size:10px;color:var(--text3);text-align:center">10 pc â†’ 1 pa &nbsp;Â·&nbsp; 5 pa â†’ 1 pe &nbsp;Â·&nbsp; 2 pe â†’ 1 po &nbsp;Â·&nbsp; 10 po â†’ 1 pp</div>
    <div style="display:flex;gap:8px;margin-top:10px">
      <button class="btn bsm" style="flex:1;color:#4caf50;border-color:rgba(76,175,80,.5);font-weight:600" onclick="openBourseModal('gagner')">ðŸ’° Gagner</button>
      <button class="btn bsm" style="flex:1;color:#e53935;border-color:rgba(229,57,53,.5);font-weight:600" onclick="openBourseModal('payer')">ðŸ’¸ Payer</button>
    </div>
  </div>
  <div class="panel">
    <div class="pt" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
      <span>Inventaire ${inv.filter(it=>it.attunement).length?`<span style="font-size:10px;padding:2px 7px;border-radius:10px;margin-left:6px;background:${attunedCount>=3?'rgba(229,57,53,.15)':'rgba(156,39,176,.1)'};color:${attunedCount>=3?'#e53935':'#9c27b0'};border:1px solid ${attunedCount>=3?'rgba(229,57,53,.4)':'rgba(156,39,176,.3)'}">ðŸ”— ${attunedCount}/3 liÃ©s</span>`:''}</span>
      ${isMJ()?`<button class="btn bsm bac" onclick="openMJCreateItem()">ðŸŽ² CrÃ©er objet/arme</button>`:''}
    </div>
    ...
    ${!ITEMS_DB?`<button class="btn bsm" style="font-size:11px;width:100%;margin-bottom:6px" onclick="loadItemsDB(()=>render())">ðŸ“š Charger le compendium d'objets</button>`:''}
...
function toggleItemCharge(itemIdx,chargeIdx){
  ...
  // Si on clique sur une charge utilisÃ©e â†’ rÃ©cupÃ©rer, sinon â†’ dÃ©penser
...
function useConsumable(itemIdx){
  ...
  if(m){
    ...
    showToast(`${item.itemType==='P'?'ðŸ§ª':'ðŸ“œ'} <strong>${esc(item.name)}</strong> : [${rolls.join('+')}]${m[3]||''} = <strong style="font-size:16px;color:var(--cp)">${total}</strong>`);
  } else {
    showToast(`${item.itemType==='P'?'ðŸ§ª':'ðŸ“œ'} <strong>${esc(item.name)}</strong> utilisÃ© !${item.desc?' â€” '+esc(item.desc.slice(0,80)):''}`);
  }
...
function addPoolItemToSac(idx){
  ...
  render();showToast(`âœ“ "${item.name}" ajoutÃ© au sac`);
}
...
function openBourseModal(type){
  ...
  const coins=[['pc','Cuivre','#b87333'],['pa','Argent','#b0bec5'],['pe','Ã‰lectrum','#6abfad'],['po','Or','var(--cp)'],['pp','Platine','#dcdcdc']];
  openModal(`<div class="pt" style="color:${isGain?'#4caf50':'#e53935'}">${isGain?'ðŸ’° Recevoir des piÃ¨ces':'ðŸ’¸ Payer'}</div>
    ...
    <div style="display:flex;gap:8px">
      ...
      <button class="btn bac" style="flex:2;font-weight:600" onclick="confirmBourse('${type}')">${isGain?'âœ“ Recevoir':'âœ“ Payer'}</button>
    </div>`);
...
function confirmBourse(type){
  ...
  if(isGain){
    ...
    if(totalPay>totalCur){showToast('âŒ Pas assez de piÃ¨ces !');return;}
  ...
  closeModal();
  saveAll();
  showToast(isGain?'ðŸ’° PiÃ¨ces reÃ§ues !':'ðŸ’¸ Paiement effectuÃ©.');
}
...
function renderSrdList(q){
  if(ITEMS_DB){
    ...
    if(!res.length)return'<div style="font-size:11px;color:var(--text3);text-align:center;padding:6px">Aucun rÃ©sultat.</div>';
    const suffix=!low&&ITEMS_DB.length>30?`<div style="font-size:10px;color:var(--text3);text-align:center;padding:4px">â€¦ et ${ITEMS_DB.length-30} autres â€” affinez la recherche</div>`:'';
    return res.map(({i,it})=>`<div class="aci" onclick="addCompendiumItem(${i})">
      <div class="ain">${_TYPE_ICON[it.t]||'ðŸ“¦'} ${esc(it.n)}${it.mg?' âœ¨':''}</div>
      <div class="ais">${esc(it.d||'')}${it.d1?' â€” '+it.d1+(it.d2?' / '+it.d2:'')+(it.dt?' '+it.dt:''):''}${it.ac?' â€” CA '+it.ac:''}</div>
    </div>`).join('')+suffix;
  }
  if(!q.trim())return[...SRD.weapons,...SRD.armors].slice(0,15).map(i=>`<div class="aci" onclick="addSrdItem('${esc(i.name)}','${esc(i.damage||i.ca||'')}','${esc(i.subtype||i.type||'')}')"><div class="ain">${esc(i.name)}</div><div class="ais">${esc(i.damage||i.ca||'')} â€” ${esc(i.price||'')}</div></div>`).join('');
  return[...SRD.weapons,...SRD.armors].filter(i=>i.name.toLowerCase().includes(q.toLowerCase())).slice(0,8).map(i=>`<div class="aci" onclick="addSrdItem('${esc(i.name)}','${esc(i.damage||i.ca||'')}','${esc(i.subtype||i.type||'')}')"><div class="ain">${esc(i.name)}</div><div class="ais">${esc(i.damage||i.ca||'')} â€” ${esc(i.price||'')}</div></div>`).join('');
}
...
function addSrdItem(name,stats,type){
  ...
  const itemType=arm?(arm.type==='Bouclier'?'S':arm.type==='LÃ©gÃ¨re'?'LA':arm.type==='IntermÃ©diaire'?'MA':'HA'):(SRD.weapons.find(w=>w.name===name)?(name.includes('Arc')||name.includes('ArbalÃ¨te')?'R':'M'):'');
  _addToInventory(p,{name,qty:1,desc:`${type} â€” ${stats}`,magic:false,linkedTo:'',itemType});render();
}
function toggleAttunement(itemIdx){
  const p=P();const item=p.inventory[itemIdx];if(!item)return;
  if(!item.attuned){
    const alreadyAttuned=(p.inventory||[]).filter(it=>it.attuned).length;
    if(alreadyAttuned>=3){showToast('âŒ Maximum 3 objets liÃ©s simultanÃ©ment. Rompez un lien d\'abord.');return;}
  }
...
function addCompendiumItem(idx){
  ...
  if(it.d1)desc+=(desc?' â€” ':'')+it.d1+(it.d2?' / '+it.d2:'')+(it.dt?' '+it.dt:'');
  if(it.ac)desc+=(desc?' â€” ':'')+'CA '+it.ac;
  if(it.d&&!desc)desc=it.d;
  const txLow=(it.tx||'').toLowerCase();
  const attunement=!!(it.mg&&(txLow.includes('nÃ©cessite un lien')||txLow.includes('requires attunement')||txLow.includes('lien avec un')||txLow.includes('attunement')));
  _addToInventory(p,{name:it.n,qty:1,desc:desc.trim(),magic:!!it.mg,linkedTo:'',itemType:it.t||'',attunement,attuned:false});
  render();showToast(`âœ“ "${it.n}" ajoutÃ© au sac`);
}
