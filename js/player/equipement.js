...
function tabEquipement(p){
  const eq=p.equip||{};
  const slotHtml=s=>{const item=eq[s.id];const isMagic=item&&item.magic;return`<div class="eq-slot${item?' filled':''}${isMagic?' magic-slot':''}" style="${isMagic?'border-color:var(--arcane)':''}" onclick="openEquipSlot('${s.id}')">
    <div class="eq-slot-label">${s.icon} ${s.label}</div>
    ${item?`<div class="eq-slot-item">${esc(item.name)}${isMagic?' ✨':''}</div><button class="btn bsm" onclick="event.stopPropagation();unequipSlot('${s.id}')" style="width:100%;margin-top:4px;color:var(--danger);border-color:rgba(229,57,53,.3);font-size:12px;padding:2px 0">✕ Retirer</button>`:`<div class="eq-slot-empty">Vide</div>`}
  </div>`;};
  ...
  return`<div>
    ...
    <div class="panel">
      <div class="pt" style="display:flex;align-items:center;justify-content:space-between;cursor:pointer" onclick="_equipProfOpen.all=!_equipProfOpen.all;render()">
        <span>Maîtrises</span>
        <span style="display:flex;align-items:center;gap:8px">${isMJ()?`<span style="font-size:12px;color:var(--cp)">🎲 toggle</span>`:''}<span style="font-size:13px;color:var(--text3)">${_equipProfOpen.all?'▴':'▾'}</span></span>
      </div>
      ${_equipProfOpen.all?`
        <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin:4px 0 6px">⚔ Armes</div>
        ${allWeapons.map(w=>{
          ...
          return`<div class="prof-row" style="${isMJ()?'cursor:pointer':''}" onclick="${isMJ()?`mjToggleWeaponProf('${jsq(w.subtype)}')`:''}" title="${isMJ()?'Cliquer pour modifier':''}">
            ...
            <span style="font-size:12px;color:var(--text3)">${esc(w.subtype)}</span>
            ${prof?`<span style="font-size:12px;color:var(--good)">✓</span>`:`<span style="font-size:12px;color:var(--danger)">✗</span>`}
          </div>`;
        }).join('')}
        <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin:10px 0 6px">🛡 Armures</div>
        ${allArmors.map(a=>{
          ...
          return`<div class="prof-row" style="${isMJ()?'cursor:pointer':''}" onclick="${isMJ()?`mjToggleArmorProf('${jsq(a.type)}')`:''}" title="${isMJ()?'Cliquer pour modifier':''}">
            ...
            <span style="font-size:12px;color:var(--text3)">${esc(a.type)}</span>
            ${prof?`<span style="font-size:12px;color:var(--good)">✓</span>`:`<span style="font-size:12px;color:var(--danger)">✗</span>`}
          </div>`;
        }).join('')}
      `:''}
