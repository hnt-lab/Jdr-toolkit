...
### ✅ FAIT (suite — v0.9.107, retour user 2026-06-14)
- **C2 v2 — BANDEAU DUR + FABs encastrés** : refonte du C2 sur retour user (« barre trop basse », « nav de modes oubliée », vision : bandeau dur où les FABs sont sertis dans des creux). Bloc relocalisé en **fin de redesign.css** (sinon écrasé par la pilule `#modeNav` définie plus bas). `#modeNav` (Hub / Personnage|MJ) devient un **gros bandeau plein-largeur** ancré en bas, liseré laiton, **safe-area-inset respectée** (corrige « trop bas »). Les 2 FABs (🎲 droite / 👥 gauche) **encastrés** dans des creux du bandeau (halo couleur-bandeau + arête sombre = bouton serti). Onglets de fiche posés **juste au-dessus** du bandeau (plus de stacking foireux avec la pilule). z 844<845<870/888.
- **DA « brute/usée »** (demande user) : (1) dé-arrondi global → arête 2px (cercles = cadrans/rivets conservés) ; (2) texture usée sur le fond (vignette + stries brossées, gradients only) ; (3) biseau « métal estampé » sur les plaques. Bloc isolé dans da-theme.css (rollback facile). 🧪 **À tester + arbitrer l'intensité** : arrondi 2px→0 ? texture +/− forte ? palette encore plus patinée ?
